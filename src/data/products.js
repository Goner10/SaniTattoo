/** @typedef {{ label: string, price: number, sku?: string }} ProductVariant */

/**
 * @typedef {{
 *   id: string
 *   sku: string
 *   name: string
 *   slug: string
 *   category: string
 *   categoryLabel: string
 *   brand: string
 *   price: number | null
 *   currency: string
 *   unit: string
 *   image: string
 *   imageSquare?: string
 *   alt: string
 *   shortDescription: string
 *   description: string | string[]
 *   tags: string[]
 *   featured: boolean
 *   offer: boolean
 *   active: boolean
 *   variants?: ProductVariant[]
 *   merchandisingFeatured?: boolean
 *   badge?: string
 *   gallery?: string[]
 *   priceIncludesVat?: boolean
 * }} Product
 */

/** @type {Product[]} */
export const products = [
  {
    id: "green-soap-cleansing-foam",
    sku: "SAN-GREEN-SOAP-FOAM-220",
    name: "Green Soap Cleaning Foam Aloe Tattoo",
    slug: "green-soap-cleansing-foam",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "aloetattoo",
    price: 8,
    currency: "EUR",
    unit: "220ml",
    image: "images/productos/green-soap-cleansing-foam.png",
    alt: "Bote de espuma limpiadora Green Soap Cleansing Foam",
    shortDescription:
      "Solución jabonosa de alta calidad de aloetattoo.",
    description:[
      "Green Soap de Aloe Tattoo es una solución jabonosa de alta calidad sin sulfatos ni PEG. Muy suave para con la piel y con gran poder de limpieza de las tintas, para utilizar directamente, sin diluir, durante el proceso del tatuaje.",
      "Su contenido en alantoína y jugo puro de aloe vera (procedente de cultivo ecológico), le confieren un alto efecto calmante y astringente (reduce el sangrado durante el tatuaje).",
      "Presentación en bote de 220ml."
    ],
    tags: ["aftercare", "green soap", "limpieza", "AloeTattoo"],
    featured: true,
    offer: false,
    active: true,
  },
  {
    id: "servilletas-ghost-tattoo",
    sku: "SAN-SERVILLETAS-GHOST-TATTOO-125",
    name: "Ghost Tattoo Wipes",
    slug: "servilletas-ghost-tattoo",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "ghost-tattoo",
    price: 7.5,
    currency: "EUR",
    unit: "125 unidades",
    image: "images/productos/ghost.png",
    alt: "Paquete de servilletas absorbentes Ghost Tattoo para estudio",
    shortDescription:
      "Servilletas super absorbentes con sistema de envasado y corte.",
      description: [
        "Nuevas servilletas para tatuajes GHOST, diseñadas para ofrecer la máxima higiene y comodidad durante el proceso de tatuaje.",
        "Las servilletas GHOST vienen en un envase protector especialmente desarrollado para evitar la contaminación cruzada, garantizando un entorno de trabajo más seguro tanto para el artista como para el cliente.",
        "El dispensador incorpora un cortador metálico de alta precisión, que permite extraer cada unidad de forma rápida, cómoda y sin tirones, optimizando el flujo de trabajo en el estudio.",
      ],
      gallery: [
        "images/productos/ghost.png",
        "images/productos/ghost-tattoo-wipes3.png",
        "images/productos/ghost-tattoo-wipes2.png",

      ],
    tags: ["material sanitario", "servilletas", "ghost tattoo", "estudio"],
    featured: true,
    offer: false,
    active: true,
  },
  {
    id:"desinfectante-ghost",
    sku: "SAN-DESINFECTANTE-GHOST-150ML",
    name: "Desinfectante Ghost Tattoo",
    slug: "desinfectante-ghost",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "ghost-tattoo",
    price: 12.35,
    currency: "EUR",
    unit: "400ml",
    image: "images/productos/ghost-desinfectante.png",
    alt: "Desinfectante Ghost Tattoo para estudio",
    shortDescription:
      "Desinfectante Ghost Tattoo para estudio.",
    description:[
      "¡Desinfectante Ghost os presenta el único aerosol que combina desinfección y limpieza en un único producto, capaz de eliminar residuos de tinta y calca!",
      "Ghost desinfectante te ofrece una limpieza y desinfección profunda y efectiva tanto en superficies como en utensilios.",
      "Autorizado para uso por el público en general con n° Reg. 20-20/40-10621, incluido en el listado de viricidas del Ministerio de Sanidad."
    ],
    tags: ["desinfección", "ghost tattoo", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "gel-stencil-premium",
    sku: "SAN-GEL-STENCIL-100",
    name: "Gel Stencil Premium Aloe Tattoo",
    slug: "gel-stencil-premium",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "aloetattoo",
    price: 12,
    currency: "EUR",
    unit: "100ml",
    image: "images/productos/stencil-premium-aloe-tattoo.png",
    alt: "Gel para stencil premium en formato bote",
    shortDescription:
      "Gel de transferencia para stencil con acabado nítido y fijación fiable.",
    description:[
      "Es un producto de alta calidad, muy fácil de extender y agradable en la piel. No necesita agitarse ni volcar el bote. Con sistema de extracción por bomba (más cómodo e higiénico). Fabricado con jugo puro de aloe vera Barbadensis Miller cultivo ecológico.",
      "MODO DE USO: El gel Stencil Premium de Aloe Tattoo es un producto de alta calidad que no precisa agitarse antes de su uso, por lo que una sola gota sirve para fijar una plantilla de dibujo semejante a un cuadrado de 15cmx15cm de lado. Extienda el gel en el área a tatuar hasta que la piel presente un aspecto casi seco. Aplique la plantilla del dibujo y espere de 3 a 5 segundos antes de retirarla. Deje pasar de 8 a 10 minutos antes de comenzar a tatuar. Trabaje de abajo a arriba, ocasionalmente limpiando el exceso de tinta. Ideal para realismos."
    ],
    tags: ["stencil", "gel", "transfer", "preparación"],
    featured: true,
    offer: false,
    active: true,
    variants: [
      { label: "100ml", price: 12, sku: "SAN-GEL-STENCIL-100" },
      { label: "220ml", price: 18, sku: "SAN-GEL-STENCIL-220" },
    ],
  },
  {
    id: "red-out-bioactive-calming-foam",
    sku: "SAN-RED-OUT-BIOACTIVE-FOAM-220",
    name: "Red Out bioactive calming foam Aloe Tattoo",
    slug: "red-out-bioactive-calming-foam",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "aloetattoo",
    price: 12,
    currency: "EUR",
    unit: "220ml",
    image: "images/productos/red_out.png",
    alt: "Bote de espuma calmante bioactiva Red Out para la piel en estudio",
    shortDescription:
      "Solucón jabonosa librre de sulfatos de Aloe Tattoo.",
    description:[
      "Solución jabonosa - libre de PEG, sulfatos y conservantes - compuesta de agentes tenso-activos de origen natural y de agentes bio-activos procedentes de la flor de la Camomila (cultivo ecológico), que le confieren a la espuma un notable efecto calmante e higienizante sobre pieles irritadas durante y al finalizar el proceso del tatuaje.",
      "Instrucciones de uso: aplicar la espuma generada por 1-2 pulsaciones sobre la piel previamente humedecida, durante las 'paradas' en el tatuaje y al finalizar el mismo. Masajear suavemente con las yemas de los dedos. Dejar actuar al producto al menos 2 minutos. Aclarar con agua y secar con toallitas de papel tissue sin frotar la piel.",
      "Presentación en bote de 220ml con dosificador de espuma."
    ],
    tags: ["aftercare", "red out", "espuma", "calmante", "piel"],
    featured: true,
    offer: false,
    active: true,
  },
  {
    id: "green-soap-aloe",
    sku: "SAN-GREEN-SOAP-ALOE-200",
    name: "Green Soap + Aloe BioTaTum",
    slug: "green-soap-aloe",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "biotatum",
    price: 5.90,
    currency: "EUR",
    unit: "250ml",
    image: "images/productos/biotatum1.png",
    alt: "Bote de espuma calmante bioactiva Red Out para la piel en estudio",
    shortDescription:
      "Jabón profesional a base de aceite de oliva y aloe vera",
    description:
      "Green Soap + Aloe es un concentrado de jabón verde natural diseñado para artistas del tatuaje. Garantiza una limpieza higiénica de la piel antes, durante y después de la sesión. Su fórmula con aceite de oliva y zumo de aloe vera elimina suavemente la tinta, la sangre y la vaselina sin irritar la piel. Los ingredientes naturales aseguran una limpieza eficaz y segura.",
    gallery: [
      "images/productos/biotatum1.png",
      "images/productos/biotatum-green-soap.jpg",
    ],
      tags: ["aftercare", "BioTaTum", "jabon", "calmante", "piel"],
    featured: true,
    offer: false,
    active: true,
  },
  {
    id: "royal-jelly-vaseline",
    sku: "SAN-ROYAL-JELLY-VASELINE-200",
    name: "Royal Jelly Aloe Vaseline BioTaTum",
    slug: "royal-jelly-vaseline",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "biotatum",
    price: 11.90,
    currency: "EUR",
    unit: "500ml",
    image: "images/productos/royal-jelly-vaseline.png",
    alt: "Vaselina profesional para tatuadores con aloe vera",
    shortDescription:
      "Vaselina profesional especialmente formulada para su uso durante el proceso del tatuaje.",
    description:
      "Enriquecida con manteca de karité (Butyrospermum Parkii Butter), zumo de hojas de aloe vera (Aloe Barbadensis Leaf Juice) y vitaminas E y A, hidrata, calma y protege la piel frente a la sequedad. No elimina el diseño transferido y ofrece una aplicación suave y cómoda. Su aroma neutro hace que el trabajo sea más agradable tanto para el artista como para el cliente.",
    gallery: [
      "images/productos/royal-jelly-vaseline.png",
      "images/productos/biotatum-royal-jelly-aloe-vaseline-.jpg",
      "images/productos/biotatum-royal-jelly-aloe-vaseline- (1).jpg",
    ],
    tags: ["aftercare", "BioTaTum", "vaselina", "piel"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "royal-jelly-vaseline-cannabis",
    sku: "SAN-ROYAL-JELLY-VASELINE-CANNABIS-200",
    name: "Royal Jelly Vaseline Cannabis BioTaTum",
    slug: "royal-jelly-vaseline-cannabis",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "biotatum",
    price: 21.90,
    currency: "EUR",
    unit: "1000ml",
    image: "images/productos/biotatum-royal-jelly-cannabis.png",
    alt: "Vaselina profesional para tatuadores con cannabis",
    shortDescription:
      "Vaselina profesional especialmente formulada para su uso durante el proceso del tatuaje.",
    description:[
      "Vaselina profesional formulada especialmente para su uso durante el proceso del tatuaje. Enriquecida con manteca de karité (Butyrospermum Parkii Butter), aceite de semilla de cáñamo (Cannabis Sativa Seed Oil) y pantenol, que nutren, calman y protegen la piel contra la sequedad.",
      "No elimina el diseño transferido ni irrita la piel. Su aroma neutro y fresco ofrece mayor comodidad durante el trabajo."
    ],
    gallery: [
      "images/productos/biotatum-royal-jelly-cannabis.png",
      "images/productos/biotatum-vaselina-royal-jelly-con-cbd-1000ml.jpg",
    ],
    tags: ["aftercare", "BioTaTum", "vaselina", "piel"],
    featured: true,
    offer: false,
    active: true,
  },
  {
    id:"copy-cream-transfer",
    sku: "SAN-COPY-CREAM-TRANSFER-200",
    name: "Copy Cream Transfer BioTaTum",
    slug: "copy-cream-transfer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "biotatum",
    price: 14.90,
    currency: "EUR",
    unit: "250ml",
    image: "images/productos/transfer.png",
    alt: "Frasco de Copy Cream Transfer para trabajo de transferencia",
    shortDescription:
      "Solución para transferencia de plantillas.",
    description:[
      "Garantiza una transferencia precisa, limpia y duradera de los diseños de tatuajes desde la plantilla a la piel: las líneas se mantienen nítidas y fáciles de seguir durante toda la sesión.",
      "Su fórmula es resistente a las manchas y a la decoloración, manteniendo la nitidez incluso durante largas sesiones de tatuaje.",
      "Apto para todo tipo de piel: suave y no irritante, ideal para pieles sensibles."
    ],
    gallery: [
      "images/productos/transfer.png",
      "images/productos/Transfer-Copy.webp",
    ],
    tags: ["aftercare", "BioTaTum", "crema", "transferencia", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"mixing-solution-biotatum",
    sku: "SAN-MIXING-SOLUTION-BIOTATUM-250",
    name: "Mixing Solution BioTaTum",
    slug: "mixing-solution-biotatum",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "biotatum",
    price: 9.90,
    currency: "EUR",
    unit: "150ml",
    image: "images/productos/mixing-solution.png",
    alt: "Frasco de Mixing Solution para trabajo de mezcla",
    shortDescription:
      "Solución para diluir tinta de tatuaje.",
    description:[
      "Solución profesional para diluir y humedecer pigmentos de tatuaje.",
      "Gracias al pantenol, ayuda a obtener la consistencia perfecta sin alterar la intensidad del color y mejora la comodidad durante el proceso de tatuado.",
      "No contiene alcohol ni ingredientes irritantes, por lo que es segura para la piel y compatible con todos los tipos de pigmentos."
    ],
    tags: ["aftercare", "BioTaTum", "mezcla", "pigmentos", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/mixing-solution.png",
    ],
  },
  {
    id:"rapid-repair-cream-biotatum",
    sku: "SAN-RAPID-REPAIR-CREAM-BIOTATUM-250",
    name: "Rapid Repair Cream BioTaTum",
    slug: "rapid-repair-cream-biotatum",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "biotatum",
    price: 6,
    currency: "EUR",
    unit: "20ml",
    image: "images/productos/rapid-repair.png",
    alt: "Frasco de Rapid Repair Cream para trabajo de reparación",
    shortDescription:
      "Cuidado intensivo y protección del tatuaje",
    description:[
        "BioTaTum Professional Rapid Repair Cream es una crema especializada para el cuidado diario de la piel tatuada. Su textura ligera y no grasa hidrata, protege y mantiene el confort de la piel, ayudando a conservar su equilibrio natural.",
        "Gracias al pantenol y la vitamina E, calma e hidrata la piel, reforzando su barrera natural y manteniendo la intensidad de los colores del tatuaje. Forma una capa protectora transpirable que evita la pérdida de humedad. Libre de colorantes y fragancias, es apta para pieles sensibles.",
      ],
    tags: ["aftercare", "BioTaTum", "crema", "reparación", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/rapid-repair.png",
      "images/productos/crema-accion-rapida-biotatum.jpg",
    ],
  },
  {
    id: "special-shading-solution-150ml",
    sku: "SAN-SPECIAL-SHADING-SOLUTION-150",
    name: "Special Shading Solution Aloe Tattoo",
    slug: "special-shading-solution-150ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "aloetattoo",
    price: 9.5,
    currency: "EUR",
    unit: "150ml",
    image: "images/productos/special-shading.png",
    alt: "Frasco de Special Shading Solution para trabajo de sombreado",
    shortDescription:
      "Solución para sombreado en formato 150 ml, uso profesional en estudio.",
    description:[
      "Special Shading Solution de Aloe Tattoo es la solución de mezcla ideal para todos los colores y marcas de tinta para tatuaje.",
      "Libre de conservantes artificiales y especialmente formulado para crear degradados en tintas negras y grises así como en color, el Special Shading Solution puede utilizarse para cualquier técnica o estilo de tatuaje."
    ],
    tags: ["aftercare", "shading", "aloetattoo", "tatuaje", "aloetattoo"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "lofty-bloom-butter-balm-150ml",
    sku: "SAN-LOFTY-BLOOM-BUTTER-BALM-150",
    name: "Lofty Bloom Butter Balm Aloe Tattoo",
    slug: "lofty-bloom-butter-balm-150ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "aloetattoo",
    price: 18,
    currency: "EUR",
    unit: "150ml",
    image: "images/productos/lofty1.png",
    alt: "Bálsamo tipo butter Lofty Bloom Butter Balm para cuidado de la piel",
    shortDescription:
      "Bálsamo de Aloe Tattoo para hidratar y cuidar la piel en fase de aftercare.",
    description:[
      "Al realizar un tatuaje, es de suma importancia controlar los procesos inflamatorios y oxidativos que se producen en la piel con el fin de acelerar la curación y minimizar las cicatrices en la zona tatuada.",
      "Para lograr esto LOFTY BLOOM Tattoo Butter Balm de Aloe Tattoo contiene únicamente ingredientes 100% naturales que combinan propiedades anti-inflamatorias y anti-oxidantes. Tocoferoles naturales (vitamina E natural)",
      "Al utilizar LOFTY BLOOM durante la realización del tatuaje, en vez de la vaselina blanca, conseguirás reducir al máximo el sangrado y la inflamación del mismo."
    ],
    tags: ["aftercare", "bálsamo", "aloetattoo", "hidratar", "piel"],
    featured: true,
    offer: false,
    active: true,
    gallery: [
      "images/productos/lofty1.png",
      "images/productos/lofty2.png",
    ],
  },
  {
    id: "stencil-eraser-spray",
    sku: "SAN-STENCIL-ERASER-SPRAY-100",
    name: "Stencil Eraser Spray  Aloe Tattoo",
    slug: "stencil-eraser-spray",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "aloetattoo",
    price: 8,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/stencil-eraser.png",
    alt: "Spray borrador de stencil en envase para estudio",
    shortDescription:
      "Spray para corregir o retirar stencil de Aloe Tattoo",
    description:[
      "Tatto stencil eraser de Aloe Tattoo es un producto creado para borrar la plantilla de dibujo sobre la piel de forma cómoda y segura, incluso hasta una hora después del secado de la plantilla. ¡Ya se terminaron los problemas a la hora de borrar el transfer: eraser de Aloe Tattoo no deja la piel irritada una vez que ha eliminado el dibujo!",
      "INSTRUCCIONES DE USO: Para obtener los mejores resultados utilice un disco desmaquillador de algodón o una compresa de gasa estéril. Frote la piel con movimientos circulares y en sentidos opuestos hasta que desaparezca el dibujo. Después limpie la piel con agua destilada, sin jabón."
    ],
    tags: ["stencil", "spray", "corrección", "preparación", "transfer"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "100ml", price: 8, sku: "SAN-STENCIL-ERASER-SPRAY-100" },
      { label: "250ml", price: 14, sku: "SAN-STENCIL-ERASER-SPRAY-250" },
    ],
  },
  {
    id:"stencil-honey-hornet",
    sku: "SAN-STENCIL-HONEY-HORNET-1U",
    name: "Stencil Honey Hornet",
    slug: "stencil-honey-hornet",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "hornet",
    price: 21,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/hornet-honey.png",
    alt: "Stencil Honey Hornet para estudio",
    shortDescription:
      "Stencil Honey Hornet para estudio.",
    description:[
      "El Stencil de Hornet, hará que tu dibujo quede pegado durante horas mientras trabajas, haciendo que no tengas que preocuparte de cómo van a quedar de definidas las líneas, ya que aguantarán mucho más de lo que imaginas.",
      "Además, cuenta con un suave aroma a miel, el cuál gustará a cualquier persona."
    ],
    tags: ["stencil", "hornet", "estudio"],
    featured: true,
    offer: false,
    active: true,
   
    gallery: [
      "images/productos/stencil-hornet-honey.png",
    ],  
  },
  {
    id:"honey-butter-hornet",
    sku: "SAN-HONEY-BUTTER-HORNET-1U",
    name: "Honey Butter Hornet",
    slug: "honey-butter-hornet",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "hornet",
    price: 18.50,
    currency: "EUR",
    unit: "220 ml",
    image: "images/productos/hornet-butter.png",
    alt: "Honey Butter Hornet para estudio",
    shortDescription:
      "Honey Butter Hornet para estudio.",
    description:[
      "Uno de los mejores Butter que podrás probar, ideal para usar mientras tatúas, ya que por su aroma a miel y textura suave permite trabajarlo cómodamente sin llegar a derretirse. Llegando a reducir la rojez e inflamación, calma la piel y evita la mezcla de pigmentos",
      "Fabricado con productos naturales, entre ellos la manteca de Karité, extracto de miel, aceite de caléndula, manteca de mango, aceite de sésamo, cera de abejas, vitamina E...."
    ],
    tags: ["aftercare", "honey butter", "hornet", "hidratar", "piel"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/hornet-butter.png",
      "images/productos/hornet-butter2.png",
    ],
  },
  {
    id:"pulverizador-hornet",
    sku: "SAN-PULVERIZADOR-HORNET-1U",
    name: "Pulverizador Hornet",
    slug: "pulverizador-hornet",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "hornet",
    price: 5.80,
    currency: "EUR",
    unit: "550 ml",
    image: "images/productos/hornet-spray.PNG",
    alt: "Pulverizador Hornet para estudio",
    shortDescription:
      "Pulverizador Hornet de 550 ml cómodo y estable.",
    description:[
      "Spray pulverizador 550 ml, de diseño cómodo y estable, debido a que el agua queda en la parte inferior, lo que lo hace más estable y cómo para ponerlo en la mesa de tatuar, en el carrito.",
      "Además, su diseño lo hace único e impactante a la vista, tanto del tatuador, como de la persona que es tatuada."
    ],
    tags: ["aftercare", "pulverizador", "hornet", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"hornet-cleaning-foam",
    sku: "SAN-HORNET-CLEANING-FOAM-1U",
    name: "Hornet Cleaning Foam",
    slug: "hornet-cleaning-foam",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "hornet",
    price: 9,
    currency: "EUR",
    unit: "220 ml",
    image: "images/productos/hornet-cleaning.PNG",
    alt: "Hornet Cleaning Foam para estudio",
    shortDescription:
      "Hornet Cleaning Foam para estudio con aroma a miel.",
    description:[
      "El foam ideal para mantener la piel hidratada y reducir el enrojecimiento.",
      "Con el inconfundible aroma a miel de hornet garantizará el confort del/la tatuador/a y del/la cliente/a."
    ],
    tags: ["aftercare", "cleaning foam", "hornet", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"hornet-honey-soap",
    sku: "SAN-HORNET-HONEY-SOAP-750ML",
    name: "Hornet Honey Soap",
    slug: "hornet-honey-soap",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "hornet",
    price: 16.90,
    currency: "EUR",
    unit: "750ml",
    image: "images/productos/hornet-honey-soap.png",
    alt: "Hornet Honey Soap para estudio",
    shortDescription:
      "Jabón concentrado elaborado con extracto de miel",
    description:[
      "Jabón concentrado elaborado con extracto de miel, su envase de cartón 100% reciclado y reciclable cumple con nuestra responsabilidad ecológica.",
      "Sin parabenos, hecho en España y no testado en animales, dilución del 10%.",
      "Envase de 750ml."
    ],
    tags: ["aftercare", "soap", "hornet", "hidratar", "piel"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/hornet-honey-soap.png",
    ],
  },
  {
    id: "proton-black-label-stencil-primer",
    sku: "SAN-PROTON-BLACK-LABEL-STENCIL-PRIMER-250ML",
    name: "Black Label - Protón Professional Stencil Primer",
    slug: "proton-black-label-stencil-primer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 22.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/blacklabel-proton.png",
    alt: "Black Label Protón Professional Stencil Primer 250 ml",
    shortDescription:
      "Stencil primer profesional de máxima fijación para sesiones largas y trabajos de alta precisión.",
    description: [
      "La versión más potente, precisa y exclusiva de nuestra línea profesional.",
      "Diseñado para artistas que exigen el máximo control, ofrece una fijación extrema y una estabilidad superior en todo tipo de pieles y sesiones largas.",
      "BLACK LABEL representa el nivel más alto de fijación y control del mercado. Ideal para trabajos de alta complejidad, líneas críticas y sesiones prolongadas, garantiza una transferencia limpia, nítida y estable.",
    ],
    tags: ["proton", "stencil", "primer", "black label", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-evolution-stencil-primer",
    sku: "SAN-PROTON-EVOLUTION-STENCIL-PRIMER-250ML",
    name: "Evolution - Protón Professional Stencil Primer",
    slug: "proton-evolution-stencil-primer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 22.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/evolution-proton.png",
    alt: "Evolution Protón Professional Stencil Primer 250 ml",
    shortDescription:
      "Stencil primer Pro Line con alta duración y transferencia limpia para trabajos exigentes.",
    description: [
      "Este nuevo producto va aún más allá, superando a sus predecesores con una duración de hasta 20 horas o más, permitiendo incluso dejar el tatuaje marcado para continuar la sesión al día siguiente.",
      "Con una textura similar a una cera, al aplicarlo sobre la piel se extiende fácilmente y permite una adherencia excepcional de la plantilla sin formar bolsas ni expandirse, gracias a su formulación magistral.",
      "Este producto garantiza una transferencia perfectamente nítida, limpia y detallada en cada tatuaje.",
    ],
    tags: ["proton", "stencil", "primer", "evolution", "pro line"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-origins-stencil-primer",
    sku: "SAN-PROTON-ORIGINS-STENCIL-PRIMER-200ML",
    name: "Protón Origins - Professional Stencil Primer",
    slug: "proton-origins-stencil-primer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 14.90,
    currency: "EUR",
    unit: "200 ml",
    image: "images/productos/proton-origins.png",
    alt: "Protón Origins Professional Stencil Primer 200 ml",
    shortDescription:
      "Stencil líquido profesional para una transferencia rápida, limpia y precisa.",
    description: [
      "protón Origins Professional Stencil Primer es un stencil líquido, que al colocarlo sobre la piel y expandirlo ves como rápidamente se extiende y el calco se pega sobre ella de una manera excepcional, no creando bolsas y evitando la expansión de la plantilla por su formulación magistral.",
    ],
    tags: ["proton", "stencil", "primer", "origins", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-stencil-remover-azul",
    sku: "SAN-PROTON-STENCIL-REMOVER-AZUL-250ML",
    name: "Protón Stencil Remover & Skin Cleanser Azul",
    slug: "proton-stencil-remover-azul",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 13.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-remover.png",
    alt: "Protón Stencil Remover & Skin Cleanser Azul 250 ml",
    shortDescription:
      "Limpiador y removedor de stencil con aroma a menta para preparar y corregir la piel.",
    description: [
      "Tu nuevo aliado imprescindible para preparar la piel y corregir cualquier error de calco durante el proceso de tatuaje.",
      "Esta fórmula avanzada ha sido desarrollada en nuestros laboratorios para ofrecer un rendimiento excepcional: elimina incluso los stencil más resistentes sin agredir la piel, dejándola perfectamente lista para recibir el nuevo calco, con olor a Menta.",
    ],
    tags: ["proton", "stencil remover", "skin cleanser", "menta", "azul"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-stencil-remover-rosa",
    sku: "SAN-PROTON-STENCIL-REMOVER-ROSA-250ML",
    name: "Protón Stencil Remover & Skin Cleanser Rosa",
    slug: "proton-stencil-remover-rosa",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 13.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-remover-rosa.png",
    alt: "Protón Stencil Remover & Skin Cleanser Rosa 250 ml",
    shortDescription:
      "Limpiador y removedor de stencil con aroma a golosina para preparar y corregir la piel.",
    description: [
      "Tu nuevo aliado imprescindible para preparar la piel y corregir cualquier error de calco durante el proceso de tatuaje.",
      "Esta fórmula avanzada ha sido desarrollada en nuestros laboratorios para ofrecer un rendimiento excepcional: elimina incluso los stencil más resistentes sin agredir la piel, dejándola perfectamente lista para recibir el nuevo calco, con olor a Golosina.",
    ],
    tags: ["proton", "stencil remover", "skin cleanser", "golosina", "rosa"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-cristal-blue-shading-solution",
    sku: "SAN-PROTON-CRISTAL-BLUE-SHADING-SOLUTION-250ML",
    name: "Protón Cristal Blue - Shading Solution",
    slug: "proton-cristal-blue-shading-solution",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 15.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-cristal-blue.png",
    alt: "Protón Cristal Blue Shading Solution 250 ml",
    shortDescription:
      "Solución cosmética astringente profesional para calmar la piel y acompañar el proceso de tatuaje.",
    description: [
      "Protón Cristal Blue Shading Solution es una solución cosmética astringente de uso profesional, desarrollada específicamente para acompañar el proceso artístico del tatuaje.",
      "Su fórmula exclusiva, basada en conocimientos magistrales tradicionales colombianos y fabricada íntegramente en España bajo estrictos controles sanitarios, está diseñada para calmar la piel durante los trabajos más exigentes.",
      "Gracias a su textura fluida y perfectamente compatible con los productos habituales en el proceso de tatuado, facilita una aplicación suave y uniforme, ayudando a mantener la piel en óptimas condiciones durante todo el procedimiento.",
      "Su fórmula avanzada reduce visiblemente el enrojecimiento, ayuda a controlar el sangrado y aporta una agradable sensación refrescante.",
      "Además, realza la intensidad de los pigmentos ya aplicados, dejando una piel luminosa, limpia y preparada para trabajar con máxima precisión.",
      "Es un aliado ideal para los artistas que buscan fluidez, control y un acabado vibrante.",
    ],
    tags: ["proton", "shading solution", "cristal blue", "azul", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-cristal-pink-shading-solution",
    sku: "SAN-PROTON-CRISTAL-PINK-SHADING-SOLUTION-250ML",
    name: "Protón Cristal Pink - Shading Solution",
    slug: "proton-cristal-pink-shading-solution",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 15.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-cristal-pink.png",
    alt: "Protón Cristal Pink Shading Solution 250 ml",
    shortDescription:
      "Solución cosmética astringente profesional para calmar la piel durante trabajos exigentes.",
    description: [
      "Protón Cristal Pink Shading Solution es una solución cosmética astringente de uso profesional, desarrollada específicamente para acompañar el proceso artístico del tatuaje.",
      "Su fórmula exclusiva, basada en conocimientos magistrales tradicionales colombianos y fabricada íntegramente en España bajo estrictos controles sanitarios, está diseñada para calmar la piel durante los trabajos más exigentes.",
      "Gracias a su textura fluida y perfectamente compatible con los productos habituales en el proceso de tatuado, facilita una aplicación suave y uniforme, ayudando a mantener la piel en óptimas condiciones durante todo el procedimiento.",
      "Su fórmula avanzada reduce visiblemente el enrojecimiento, ayuda a controlar el sangrado y aporta una agradable sensación refrescante.",
      "Además, realza la intensidad de los pigmentos ya aplicados, dejando una piel luminosa, limpia y preparada para trabajar con máxima precisión.",
      "Es un aliado ideal para los artistas que buscan fluidez, control y un acabado vibrante.",
    ],
    tags: ["proton", "shading solution", "cristal pink", "rosa", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-pink-foamer",
    sku: "SAN-PROTON-PINK-FOAMER-220ML",
    name: "Pink Foamer Protón",
    slug: "proton-pink-foamer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 7.90,
    currency: "EUR",
    unit: "220 ml",
    image: "images/productos/proton-pink-foamer.png",
    alt: "Pink Foamer Protón espuma limpiadora 220 ml",
    shortDescription:
      "Espuma limpiadora con pantenol, alantoína y aloe vera, con aroma a golosina.",
    description: [
      "Pink Foamer Protón es el espumizador revolucionario que incorpora el Soap más avanzado del mercado, formulado con ingredientes bioactivos de la más alta calidad.",
      "Enriquecido con pantenol, alantoína y aloe vera, este espumizador no solo limpia profundamente, sino que también cuida y repara la piel del cliente, proporcionando un alivio inmediato durante el proceso de tatuaje.",
      "Su delicado aroma a golosina añade un toque agradable y reconfortante a la experiencia de tus clientes.",
    ],
    tags: [
      "proton",
      "foamer",
      "espuma limpiadora",
      "pantenol",
      "alantoina",
      "aloe vera",
      "rosa",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-blue-foamer",
    sku: "SAN-PROTON-BLUE-FOAMER-220ML",
    name: "Blue Foamer Protón",
    slug: "proton-blue-foamer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 7.90,
    currency: "EUR",
    unit: "220 ml",
    image: "images/productos/proton-green-foamer.png",
    alt: "Blue Foamer Protón espuma limpiadora 220 ml",
    shortDescription:
      "Espuma limpiadora con pantenol, alantoína y aloe vera, con aroma a menta.",
    description: [
      "Blue Foamer Protón es el espumizador revolucionario que incorpora el Soap más avanzado del mercado, formulado con ingredientes bioactivos de la más alta calidad.",
      "Enriquecido con pantenol, alantoína y aloe vera, este espumizador no solo limpia profundamente, sino que también cuida y repara la piel del cliente, proporcionando un alivio inmediato durante el proceso de tatuaje.",
      "Su delicado aroma a menta añade un toque agradable, refrescante y reconfortante a la experiencia de tus clientes.",
    ],
    tags: [
      "proton",
      "foamer",
      "espuma limpiadora",
      "pantenol",
      "alantoina",
      "aloe vera",
      "menta",
      "azul",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-green-foamer",
    sku: "SAN-PROTON-GREEN-FOAMER-220ML",
    name: "Green Foamer Protón",
    slug: "proton-green-foamer",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 7.90,
    currency: "EUR",
    unit: "220 ml",
    image: "images/productos/proton-blue-foamer.png",
    alt: "Green Foamer Protón espuma limpiadora 220 ml",
    shortDescription:
      "Espuma limpiadora con pantenol, alantoína y aloe vera, con aroma a melón.",
    description: [
      "Green Foamer Protón es el espumizador revolucionario que incorpora el Soap más avanzado del mercado, formulado con ingredientes bioactivos de la más alta calidad.",
      "Enriquecido con pantenol, alantoína y aloe vera, este espumizador no solo limpia profundamente, sino que también cuida y repara la piel del cliente, proporcionando un alivio inmediato durante el proceso de tatuaje.",
      "Su delicado aroma a melón añade un toque agradable y reconfortante a la experiencia de tus clientes.",
    ],
    tags: [
      "proton",
      "foamer",
      "espuma limpiadora",
      "pantenol",
      "alantoina",
      "aloe vera",
      "melon",
      "verde",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-pink-soap-concentrado",
    sku: "SAN-PROTON-PINK-SOAP-CONCENTRADO-1L",
    name: "Pink Soap Protón Concentrado",
    slug: "proton-pink-soap-concentrado",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 8.90,
    currency: "EUR",
    unit: "1 L",
    image: "images/productos/proton-pink-soap.png",
    alt: "Pink Soap Protón concentrado 1 litro",
    shortDescription:
      "Jabón concentrado con pantenol, alantoína y aloe vera, con fragancia dulce de fresa.",
    description: [
      "Limpia, protege y calma la piel con una fórmula avanzada que combina ciencia y naturaleza.",
      "Diseñado para ofrecer la máxima eficacia durante el proceso de tatuaje, este jabón concentrado rosa es todo lo que necesitas para garantizar una experiencia cómoda y profesional.",
      "Su fragancia dulce de fresa aporta un toque fresco y delicioso, haciendo que cada sesión sea más placentera tanto para el cliente como para el artista.",
    ],
    tags: [
      "proton",
      "soap",
      "jabon concentrado",
      "pantenol",
      "alantoina",
      "aloe vera",
      "fresa",
      "rosa",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-blue-soap-concentrado",
    sku: "SAN-PROTON-BLUE-SOAP-CONCENTRADO-1L",
    name: "Blue Soap Protón Concentrado",
    slug: "proton-blue-soap-concentrado",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 8.90,
    currency: "EUR",
    unit: "1 L",
    image: "images/productos/proton-blue-soap.png",
    alt: "Blue Soap Protón concentrado 1 litro",
    shortDescription:
      "Jabón concentrado con pantenol, alantoína y aloe vera, con aroma a menta.",
    description: [
      "Limpia, protege y calma la piel con una fórmula avanzada que combina ciencia y naturaleza.",
      "Diseñado para ofrecer la máxima eficacia durante el proceso de tatuaje, este jabón concentrado rosa es todo lo que necesitas para garantizar una experiencia cómoda y profesional.",
      "Su ligero aroma a menta aporta frescor tanto al cliente como al artista, creando una experiencia agradable en cada sesión.",
    ],
    tags: [
      "proton",
      "soap",
      "jabon concentrado",
      "pantenol",
      "alantoina",
      "aloe vera",
      "menta",
      "azul",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-green-soap-concentrado",
    sku: "SAN-PROTON-GREEN-SOAP-CONCENTRADO-1L",
    name: "Green Soap Protón Concentrado",
    slug: "proton-green-soap-concentrado",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 8.90,
    currency: "EUR",
    unit: "1 L",
    image: "images/productos/proton-green-soap.png",
    alt: "Green Soap Protón concentrado 1 litro",
    shortDescription:
      "Jabón concentrado con pantenol, alantoína y aloe vera, con fragancia de melón.",
    description: [
      "Limpia, protege y calma la piel con una fórmula avanzada que combina ciencia y naturaleza.",
      "Diseñado para ofrecer la máxima eficacia durante el proceso de tatuaje, este jabón concentrado rosa es todo lo que necesitas para garantizar una experiencia cómoda y profesional.",
      "Su fragancia de melón fresco aporta un toque dulce y refrescante que hará más placentera cada sesión tanto para el cliente como para el artista.",
    ],
    tags: [
      "proton",
      "soap",
      "jabon concentrado",
      "pantenol",
      "alantoina",
      "aloe vera",
      "melon",
      "verde",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-tattoo-cream-made-by-nature",
    sku: "SAN-PROTON-TATTOO-CREAM-250ML",
    name: "Protón Tattoo Cream - Made by Nature",
    slug: "proton-tattoo-cream-made-by-nature",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 19.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-tattoo-cream.png",
    alt: "Protón Tattoo Cream Made by Nature 250 ml",
    shortDescription:
      "Manteca para usar durante el tatuaje, con extractos naturales y textura controlada.",
    description: [
      "Es la mejor manteca que puedes usar mientras tatuas por su contenido en Extractos Naturales.",
      "Protón Tattoo Cream tiene una textura suave y aceitosa, pero sin derretirse cuando la colocamos encima del guante para ir agregando a la piel poco a poco.",
      "Contiene mantecas de Karite y Mango, además de extractos de Ortiga, Liquen de Islandia, Calendula, Aceite de Sésamo y otros aceites esenciales, para ir regenerando la piel desde el minuto 0.",
      "Su aroma exclusivo no te dejará indiferente.",
    ],
    tags: ["proton", "tattoo cream", "butter", "manteca", "made by nature", "aftercare"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-origins-process-butter",
    sku: "SAN-PROTON-ORIGINS-PROCESS-BUTTER-250ML",
    name: "Protón Origins - Process Butter",
    slug: "proton-origins-process-butter",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 13.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-process-butter.png",
    alt: "Protón Origins Process Butter 250 ml",
    shortDescription:
      "Manteca original de Protón para el proceso de tatuaje, con textura suave y aroma característico.",
    description: [
      "Es la manteca ORIGINAL de Protón, la que lo empezó todo.",
      "Diseñada para el proceso de tatuaje (y para que la piel lo agradezca desde el primer minuto), combina una selección premium de aceites esenciales y Vitamina E para mantener la piel más elástica, más cómoda y mejor cuidada mientras trabajas.",
      "Su olor característico a coco… y algo más está importado directamente desde Colombia, y convierte cada pasada en una experiencia.",
      "Su textura es suave y aceitosa, perfecta para deslizar sin esfuerzo… pero con un detalle clave, no se derrite sobre el guante.",
      "Así puedes aplicarla poco a poco, con control, sin desperdicio y sin “charcos” innecesarios.",
    ],
    tags: ["proton", "origins", "process butter", "manteca", "coco", "tatuaje"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-cbd-tattoo-butter",
    sku: "SAN-PROTON-CBD-TATTOO-BUTTER-250ML",
    name: "CBD - Proton Tattoo Butter",
    slug: "proton-cbd-tattoo-butter",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 25.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-cbd-butter.png",
    alt: "CBD Proton Tattoo Butter Made by Nature 250 ml",
    shortDescription:
      "Tattoo butter con CBD, extractos naturales y textura controlada para el proceso de tatuado.",
    description: [
      "Descubre una de las formulaciones más prodigiosas y únicas de los laboratorios de Protón Europa: CBD Protón Tattoo Butter.",
      "Esta crema se ha diseñado meticulosamente para ofrecer la mejor experiencia durante el proceso de tatuado, gracias a su contenido de extractos naturales y su pureza excepcional.",
      "Beneficios del Producto:",
      "• Alta Concentración de CBD: Cada bote de CBD Protón Tattoo Butter contiene 800 mg de CBD aislado con una pureza del 99.9%, proveniente de Colombia. Esta alta concentración asegura un efecto verdaderamente satisfactorio y relajante durante el tatuaje. Y su posterior cicatrización.",
      "• Ingredientes Naturales: Nuestra fórmula exclusiva se ha desarrollado utilizando únicamente materiales obtenidos directamente de la naturaleza, garantizando la más alta calidad y seguridad.",
      "• Textura Ideal: La crema tiene una textura suave y aceitosa, pero está formulada para no derretirse cuando se coloca sobre el guante, permitiendo una aplicación controlada y gradual sobre la piel.",
      "• Cuidado Integral de la Piel: Enriquecida con mantecas de karité y mango, así como extractos de caléndula y aceite de sésamo, esta crema regenera la piel desde el primer momento. Además, contiene otros aceites esenciales que potencian su efecto regenerador.",
      "• Aroma Exclusivo: El aroma distintivo de CBD Proton Tattoo Butter es una característica que no te dejará indiferente, añadiendo una dimensión sensorial agradable a la experiencia del tatuaje.",
      "• Libre de Petroquímicos: Nuestra CBD Tattoo Butter está totalmente libres de aceites minerales ni derivados del petroleo. Solo ingredientes naturales.",
    ],
    tags: ["proton", "cbd", "tattoo butter", "manteca", "made by nature", "aftercare"],
    featured: true,
    offer: false,
    active: true,
  },
  {
    id: "proton-colours-obsession-tattoo-butter",
    sku: "SAN-PROTON-COLOURS-OBSESSION-TATTOO-BUTTER-250ML",
    name: "Protón Colours Obsession - Tattoo Butter",
    slug: "proton-colours-obsession-tattoo-butter",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 19.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-coors-obsession.png",
    alt: "Protón Colours Obsession Tattoo Butter 250 ml",
    shortDescription:
      "Tattoo butter disponible en varios colores, aromas y extractos específicos.",
    description: [
      "Creada con 7 Colores diferentes.",
      "Pero no solo son colores... Cada Mantenca tiene unos aceites esenciales y extractos relacionados con su Olor y Color.",
      "Una manteca Excepcional con un tacto sublime para poder tatuar horas y horas sin irritar la piel de los clientes.",
      "Protón Colours Obsession tiene una textura suave y aceitosa, pero sin derretirse cuando la colocamos encima del guante para ir agregando a la piel poco a poco.",
      "Regenerando la piel desde el minuto 0 aportando unos aromas y extractos únicos.",
      "• Naranja: Aroma de Naranja, Extracto de Naranja y Aceite esencial de naranja Amarga.",
      "• Violeta: Aroma de Mora y Extracto de Violeta de Genciana.",
      "• Verde: Aroma de Melon, Extracto de Pepino.",
      "• Negro: Aroma de Vainilla y Tabaco y extracto de Uva.",
      "• Amarillo: Aroma de Limon, Aceite esencial de Limon y extracto de Corteza de Limon.",
      "• Rosa: Aroma de Chicle, y extracto de Granada.",
      "• Azul: Aroma de Menta, extracto de Te Azul Oolong y aceite esencial de menta piperina.",
    ],
    variants: [
      {
        label: "Naranja - Aroma a naranja",
        price: 19.90,
        sku: "SAN-PROTON-COLOURS-OBSESSION-NARANJA-250ML",
      },
      {
        label: "Violeta - Aroma de mora",
        price: 19.90,
        sku: "SAN-PROTON-COLOURS-OBSESSION-VIOLETA-250ML",
      },
      {
        label: "Verde - Aroma de melón",
        price: 19.90,
        sku: "SAN-PROTON-COLOURS-OBSESSION-VERDE-250ML",
      },
      {
        label: "Negro - Aroma a vainilla y tabaco",
        price: 19.90,
        sku: "SAN-PROTON-COLOURS-OBSESSION-NEGRO-250ML",
      },
      {
        label: "Amarillo - Aroma a limón",
        price: 19.90,
        sku: "SAN-PROTON-COLOURS-OBSESSION-AMARILLO-250ML",
      },
      { label: "Rosa - Aroma a chicle", price: 19.90, sku: "SAN-PROTON-COLOURS-OBSESSION-ROSA-250ML" },
      { label: "Azul - Aroma a menta", price: 19.90, sku: "SAN-PROTON-COLOURS-OBSESSION-AZUL-250ML" },
    ],
    tags: [
      "proton",
      "colours obsession",
      "tattoo butter",
      "manteca",
      "colores",
      "aftercare",
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-photo-finish",
    sku: "SAN-PROTON-PHOTO-FINISH-200ML",
    name: "Photo Finish - Protón",
    slug: "proton-photo-finish",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 17.90,
    currency: "EUR",
    unit: "200 ml",
    image: "images/productos/proton-finish-photo.png",
    alt: "Photo Finish Protón 200 ml",
    shortDescription:
      "Gel transparente de acabado para aplicar al finalizar el tatuaje y mejorar el resultado visual.",
    description: [
      "Photo Finish Protón es el finish premium, un gel transparente de consistencia media diseñado para aplicarse justo al finalizar el tatuaje y conseguir un acabado visual impecable.",
      "Su combinación de Aloe Vera, Glicerina y Pantenolactúa de forma inmediata sobre la piel: reduce la rojez, disminuye el sangrado visible, realza el contraste y la saturación y deja el tatuaje listo para fotos nítidas, limpias y espectaculares.",
    ],
    tags: ["proton", "photo finish", "finish", "gel", "tatuaje", "aftercare"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "proton-liquid-solidifier",
    sku: "SAN-PROTON-LIQUID-SOLIDIFIER-250ML",
    name: "Liquid Solidifier Protón",
    slug: "proton-liquid-solidifier",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "proton",
    price: 13.90,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/proton-liquid.png",
    alt: "Liquid Solidifier Protón 250 ml",
    shortDescription:
      "Polvo solidificante profesional para convertir líquidos en sólido estable en segundos.",
    description: [
      "Protón Liquid Solidifier es un polvo solidificante de grado profesional, diseñado para estudios de tatuaje y piercing que trabajan bajo los más altos estándares de higiene, control y responsabilidad ambiental.",
      "Su fórmula granular de acción rápida, convierte cualquier líquido en un sólido estable en cuestión de segundos, permitiendo una eliminación limpia, segura y completamente controlada.",
      "Ideal para: restos de tinta, agua de limpieza de agujas y líquidos utilizados durante el procedimiento.",
    ],
    tags: ["proton", "liquid solidifier", "solidificante", "higiene", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-thermal-paper",
    sku: "SAN-REAL-THERMAL-PAPER-100",
    name: "Real Thermal Paper",
    slug: "real-thermal-paper",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 38.90,
    currency: "EUR",
    unit: "100 hojas",
    image: "images/productos/real-thermal-paper.PNG",
    alt: "Real Thermal Paper 100 unidades",
    shortDescription:
      "Papel termotransferible para impresión de stencils.",
    description:
      "Papel de primera calidad para diseños de tatuaje, ideal para usarse con impresora térmica.",
    tags: ["material sanitario", "stencil", "impresión", "papel"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-thermal-roll",
    sku: "SAN-REAL-THERMAL-ROLL-100",
    name: "Real Thermal Roll 21x30cm",
    slug: "real-thermal-roll",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 35,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/real-thermal-roll1.png",
    alt: "Real Thermal Roll 100 unidades",
    shortDescription:
      "Roll de papel termotransferible para impresión de stencils.",
    description:[
      "Presentamos el nuevo rollo de papel thermal de la marca REAL diseñado para aquellos artistas que necesitan sus plantillas con la mayor nitidez posible y una resistencia máxima en cada calco.",
      "Con un tamaño de 21 centímetros de ancho por 30 metros de largo lo hace compatible con la mayoría de las termocopiadoras existentes del mercado."  
    ],
    gallery: [
      "images/productos/real-thermal-roll1.png",
      "images/productos/real-thermal-roll2.png",
      "images/productos/real-thermal-roll3.png",
    ],
    tags: ["material sanitario", "stencil", "impresión", "papel"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-brush-marker",
    sku: "SAN-REAL-BRUSH-MARKER-1U",
    name: "Real Brush Marker - Bolígrafo doble para freehand",
    slug: "real-brush-marker",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 2.70,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/real-boligrafo.PNG",
    alt: "Real Brush Marker 1 unidad",
    shortDescription:
      "Bolígrafo doble para freehand, con tinta de color violeta y punta fina para trazos precisos.",
    description:[
      "Este Bolígrafo se compone de una punta súper finita y otra más ancha para trazos más anchos.",
      "De un color violeta perfecto para freehand y retocar calcos sobre la piel."
    ],
    tags: ["material sanitario", "stencil", "impresión", "brocha"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-freehand-ink",
    sku: "SAN-REAL-FREEHAND-INK-100ML",
    name: "Tinta hectográfica violeta de genciana - Real Freehand Ink",
    slug: "real-freehand-ink",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 20.00,
    currency: "EUR",
    unit: "120 ml",
    image: "images/productos/real-tinta.PNG",
    alt: "Real Freehand Ink 100 ml",
    shortDescription:
      "Tinta hectográfica de genciana para freehand, con color violeta y punta fina para trazos precisos.",
    description:
      "Tinta Hectográfica REAL FREEHAND diseñada para crear a mano alzada y crean tus propias plantillas. Disponible en color violeta.",
    tags: ["material sanitario", "stencil", "impresión", "brocha"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-stencil-printer",
    sku: "SAN-REAL-STENCIL-PRINTER-1U",
    name: "Real Stencil Printer",
    slug: "real-stencil-printer",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 56.90,
    currency: "EUR",
    unit: "120 ml",
    image: "images/productos/real-stencil-printer.png",
    alt: "Real Stencil Printer 120 ml",
    shortDescription:
      "Tinta hectográfica compatible con impresoras ECO TANK.",
    description:[
      "Tinta hectográfica compatible con impresoras ECO TANK.",
      "Envase de 120 ml",
    ],
  
    tags: ["material sanitario", "stencil", "impresión"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-stencil-true-crystal-clear",
    sku: "SAN-REAL-STENCIL-TRUE-CRYSTAL-CLEAR-100U",
    name: "Real Stencil True Crystal Clear",
    slug: "real-stencil-true-crystal-clear",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 39.90,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/real-stencil-true-crystal-clear.png",
    alt: "Real Stencil True Crystal Clear 100 unidades",
    shortDescription:
      "Real Stencil True Crystal Clear 100 unidades",
    description:[
      "Es el papel transparente ideal para tus plantillas. Úsalo con tu termocopiadora habitual. ",
      "Pack de 100 unidades."],
    tags: ["material sanitario", "stencil", "impresión"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-stencil-paper",
    sku: "SAN-REAL-STENCIL-PAPER-500U",
    name: "Real Stencil Paper",
    slug: "real-stencil-paper",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 35.50,
    currency: "EUR",
    unit: "500 unidades",
    image: "images/productos/real-stencil-paper.png",
    alt: "Real Stencil Paper 500 unidades",
    shortDescription:
      "Real Stencil Paper 500 unidades",
    description:[
      "Es el papel ideal para imprimir plantillas con nuestra tinta hectográfica Real Stencil Printer. Pero también válido para utilizar en termocopiadora. ",
      "Pack de 500 unidades."],
    tags: ["material sanitario", "stencil", "impresión"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"real-mapping-string",
    sku: "SAN-REAL-MAPPING-STRING-30M",
    name: "Real Mapping String",
    slug: "real-mapping-string",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "real-stencil-printer",
    price: 6.50,
    currency: "EUR",
    unit: "30 metros",
    image: "images/productos/real-mapping-string.png",
    alt: "Real Mapping String 30 metros",
    shortDescription:
      "Real Mapping String 30 metros",
    description:[
      "El marcador REAL Mapping String viene listo y pre-entintado para realizar líneas perfectas sobre la piel y obtener las referencias necesarias para realizar un trabajo preciso.",
      "Perfecto para guiarte a la hora de realizar distintos tipos de tatuajes, gracias a sus 30 metros de longitud y la cuchilla de corte precisa puedes elegir el tamaño de hilo que requieres para tu sesión.",
    "Para un marcado más duradero del producto dejar secar el trazo un rato tras su uso."
    ],
    gallery: [
      "images/productos/real-mapping-string.png",
      "images/productos/hilo.jpeg",
    ],
    tags: ["material sanitario", "stencil", "impresión"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"yummy-stencil-180",
    sku: "SAN-YUMMY-STENCIL-180",
    name: "Stencil Yummy 180",
    slug: "yummy-stencil-180",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "yummy",
    price: 13.90,
    currency: "EUR",
    unit: "180 ml",
    image: "images/productos/yummy-stencil.png",
    alt: "Stencil Yummy 180",
    shortDescription:
      "Stencil Yummy 180ml.",
    description:[
      "Déjate conquistar por el aroma que desprende el nuevo Stencil de la marca YUMMY, transpórtate hasta tu infancia en cada calco con el olor a chicle que desprende.",
      "Además, este bonito osito cuenta con una textura líquida y suave que hará de cada sesión una experencia en si misma."
    ],
    tags: ["material sanitario", "stencil", "impresión"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "tinta-hectografica-sunnink-120ml",
    sku: "SAN-SUNNINK-TINTA-HECTO-120",
    name: "Tinta Hectográfica Sunnink",
    slug: "tinta-hectografica-sunnink-120ml",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "sunnink",
    price: 66.03,
    currency: "EUR",
    unit: "120 ml",
    image: "images/productos/sunnink-tinta.png",
    alt: "Tinta hectográfica Sunnink 120 ml para impresoras EcoTank",
    shortDescription:
      "Tinta hectográfica para impresoras EcoTank y plantillas de alta calidad.",
    description: [
      "La tinta hectográfica está diseñada para impresoras EcoTank y permite imprimir plantillas de alta calidad para tatuajes.",
      "La tinta es especialmente formulada para producir imágenes precisas en papel de una sola capa, facilitando el proceso de tatuaje con un coste reducido por página en comparación con otros métodos tradicionales.",
    ],
    tags: ["sunnink", "tinta hectográfica", "stencil", "impresión", "ecotank"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "butter-sunnink-250ml",
    sku: "SAN-SUNNINK-BUTTER-250",
    name: "Butter Sunnink",
    slug: "butter-sunnink-250ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 20.58,
    currency: "EUR",
    unit: "250 ml",
    image: "images/productos/sunnink-sweet-buter.png",
    alt: "Manteca Butter Sunnink 250 ml para tatuaje",
    shortDescription:
      "Manteca formulada con Tsubaki, mango y karité para hidratar y lubricar la piel.",
    description: [
      "Es una manteca especialmente formulada para tatuajes que contiene extractos naturales de Tsubaki, además de manteca de mango y manteca de karité.",
      "Esta combinación de ingredientes proporciona propiedades hidratantes, regeneradoras y calmantes para la piel durante el proceso de tatuaje.",
      "La manteca ayuda a preparar la piel antes del tatuaje, facilita la lubricación durante el tatuaje para un deslizamiento suave de las agujas, y apoya la cicatrización y cuidado posterior, manteniendo la piel hidratada, flexible y protegida.",
      "Con un aroma increíble que hará la sesión de tatuaje agradable.",
    ],
    tags: ["sunnink", "butter", "manteca", "aftercare", "hidratación"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/sunnink-sweet-buter.png",
      "images/productos/sunnink-sweet-buter2.png",
    ],
  },
  {
    id: "foam-sunnink-220ml",
    sku: "SAN-SUNNINK-FOAM-220",
    name: "Foam Sunnink",
    slug: "foam-sunnink-220ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 11.49,
    currency: "EUR",
    unit: "220 ml",
    image: "images/productos/sunnink-foam.png",
    alt: "Espuma limpiadora Foam Sunnink 220 ml",
    shortDescription:
      "Espuma para limpieza de la piel antes y durante el tatuaje.",
    description: [
      "La espuma Foam de Sunnink es un producto especialmente formulado para la limpieza de la piel antes y durante el proceso de tatuaje, con ingredientes activos que ayudan a reducir irritación y minimizar la rojez.",
      "Está diseñada para limpiar suavemente sin irritar, eliminando restos de tinta y exceso de pigmento.",
    ],
    tags: ["sunnink", "foam", "espuma", "limpieza", "aftercare"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "stencil-sunnink-airless-200ml",
    sku: "SAN-SUNNINK-STENCIL-AIRLESS-200",
    name: "Stencil Sunnink Airless",
    slug: "stencil-sunnink-airless-200ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 20.58,
    currency: "EUR",
    unit: "200 ml",
    image: "images/productos/sunnink-stencil-gel.png",
    alt: "Stencil Sunnink Airless 200 ml en envase airless",
    shortDescription:
      "Stencil en gel airless con adhesión fuerte y textura de rápida absorción.",
    description: [
      "El nuevo stencil de Sunnink está especialmente diseñado para el pegado de plantillas en la piel durante el proceso de tatuaje, destacando por su fórmula mejorada que ofrece una adhesión mucho más fuerte y duradera.",
      "Además, el producto cuenta con una textura en gel de rápida absorción que facilita su aplicación y proporciona un tacto agradable.",
      "Su aroma es otra característica destacada, con una fragancia increíble que mejora la experiencia tanto para el artista como para el cliente.",
    ],
    tags: ["sunnink", "stencil", "gel", "transfer", "preparación"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "sunnink-freehand-60ml",
    sku: "SAN-SUNNINK-FREEHAND-60",
    name: "Sunnink Freehand",
    slug: "sunnink-freehand-60ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 10.66,
    currency: "EUR",
    unit: "60 ml",
    image: "images/productos/sunnink-freehand.png",
    alt: "Tinta freehand Sunnink 60 ml",
    shortDescription:
      "Tinta para patrones pintados a mano duraderos y precisos. No apta para impresoras.",
    description: [
      "Permite crear patrones pintados a mano duraderos y precisos.",
      "Aplicar directamente sobre la piel con una brocha o bolígrafo recargable.",
      "NO APTA PARA IMPRESORAS.",
    ],
    tags: ["sunnink", "freehand", "stencil", "tinta", "manual"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "pink-foam-concentrate-sunnink-500ml",
    sku: "SAN-SUNNINK-PINK-SOAP-500",
    name: "Pink Foam Concentrate Sunnink",
    slug: "pink-foam-concentrate-sunnink-500ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 14.01,
    currency: "EUR",
    unit: "500 ml",
    image: "images/productos/sunnink-soap.png",
    alt: "Pink Foam Concentrate Sunnink 500 ml",
    shortDescription:
      "Jabón concentrado con pantenol, aloe vera y caléndula para limpieza e hidratación.",
    description: [
      "El Pink Soap Concentrado de Sunnink está formulado específicamente para el cuidado de la piel durante y después del tatuaje, ofreciendo una limpieza eficaz sin comprometer la hidratación ni la regeneración cutánea.",
      "Su fórmula avanzada combina ingredientes cuidadosamente seleccionados como el pantenol, el aloe vera y la caléndula, conocidos por sus propiedades calmantes, hidratantes y regeneradoras. Esto lo convierte en un aliado esencial para reducir la irritación, mantener la piel en óptimas condiciones y favorecer el proceso de recuperación.",
      "Además, incorpora un agradable aroma a piruleta roja, que mejora la experiencia tanto del tatuador como del cliente, aportando una sensación fresca y distintiva durante todo el proceso.",
    ],
    tags: ["sunnink", "pink soap", "green soap", "limpieza", "aftercare"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "sunnink-draft-borrador-200ml",
    sku: "SAN-SUNNINK-DRAFT-200",
    name: "Sunnink Draft Borrador y Preparador de piel",
    slug: "sunnink-draft-borrador-200ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 12.31,
    currency: "EUR",
    unit: "200 ml",
    image: "images/productos/sunnink-stencil-remove.png",
    alt: "Sunnink Draft borrador de plantillas 200 ml",
    shortDescription:
      "Borrador de plantillas en pulverizador, rápido y efectivo.",
    description: [
      "El borrador de plantillas para la piel de Sunnink funciona con un solo pulverizador sobre la plantilla en la piel; se deja actuar durante 60 segundos para borrar completamente la plantilla.",
      "En caso de que queden rastros, se recomienda repetir el proceso hasta que se elimine todo el dibujo.",
      "Este método es rápido y efectivo para borrar las plantillas hechas con la tinta hectográfica de Sunnink.",
    ],
    tags: ["sunnink", "borrador", "stencil", "preparación", "draft"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "gel-film-bioactivo-sunnink-10ml",
    sku: "SAN-SUNNINK-GEL-FILM-10",
    name: "Gel Film Bioactivo Sunnink",
    slug: "gel-film-bioactivo-sunnink-10ml",
    category: "cuidados-tatuaje",
    categoryLabel: "Cuidados tatuaje",
    brand: "sunnink",
    price: 16.45,
    currency: "EUR",
    unit: "10 ml",
    image: "images/productos/sunnink-gel-bio.png",
    alt: "Gel Film Bioactivo Sunnink 10 ml post-tatuaje",
    shortDescription:
      "Gel post-tatuaje que crea una película protectora flexible e invisible.",
    description: [
      "Gel post-tatuaje profesional que crea una película protectora flexible e invisible.",
      "Diseñado para aplicarse inmediatamente después de la sesión, ayudando a proteger la zona y mantener la hidratación.",
    ],
    tags: ["sunnink", "gel film", "aftercare", "post-tatuaje", "hidratación"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"impresora-epson-ecotank",
    sku: "SAN-IMPRESORA-EPSON-ECOTANK-1U",
    name: "Impresora Epson EcoTank",
    slug: "impresora-epson-ecotank",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "sunnink",
    price: 157.02,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/impresora.png",
    alt: "Impresora Epson EcoTank 1 unidad",
    shortDescription:
      "Impresora Epson EcoTank para impresión de stencils y tinta hectográfica.",
    description: [
      "La impresora que recomendamos para usar con la tinta hectográfica Sunnink.",
    ],
    tags: ["impresora", "epson", "ecotank", "stencil", "tinta hectográfica"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "guantes-nitrilo-100",
    sku: "SAN-GUANTES-NITRILO-NEGRO-100",
    name: "Guantes de nitrilo",
    slug: "guantes-nitrilo-100",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 3.5,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/guantes-nitrilo.png",
    alt: "Caja de guantes de nitrilo desechables para estudio de tatuaje",
    shortDescription:
      "Guantes desechables de nitrilo en caja de 100 unidades. Disponibles en varios colores.",
    description:
      "Guantes de nitrilo sin polvo pensados para protección e higiene en sesión. Buen ajuste y tacto para trabajo preciso. En colores negro y violeta. Caja de 100 unidades.",
    tags: ["material sanitario", "protección", "higiene", "guantes", "nitrilo", "desechables"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "Talla S", price: 3.5, sku: "SAN-GUANTES-NITRILO-NEGRO-100" },
      { label: "Talla M", price: 3.5, sku: "SAN-GUANTES-NITRILO-AZUL-100" },
      { label: "Talla L", price: 3.5, sku: "SAN-GUANTES-NITRILO-VIOLETA-100" },
      { label: "Talla XL", price: 3.5, sku: "SAN-GUANTES-NITRILO-XL-100" },
    ],
  },
  {
    id: "guantes-latex-sin-polvo-100",
    sku: "SAN-GUANTES-LATEX-SP-100",
    name: "Guantes de látex sin polvo",
    slug: "guantes-latex-sin-polvo-100",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 4.8,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/guantes-latex.png",
    alt: "Caja de guantes de látex sin polvo para uso profesional",
    shortDescription:
      "Guantes de látex sin polvo en caja de 100 unidades para protección en estudio.",
    description:
      "Guantes de látex sin polvo para uso profesional. Ideales cuando buscas una alternativa al nitrilo manteniendo buena sensibilidad táctil.",
    tags: ["protección", "higiene", "guantes", "látex", "sin polvo"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "Talla S", price: 4.8, sku: "SAN-GUANTES-NITRILO-S-100" },
      { label: "Talla M", price: 4.8, sku: "SAN-GUANTES-NITRILO-M-100" },
      { label: "Talla L", price: 4.8, sku: "SAN-GUANTES-NITRILO-L-100" },
    ],
  },
  {
    id: "rasuradoras-doble-hoja-100",
    sku: "SAN-RASURADORAS-DOBLE-HOJA-100",
    name: "Rasuradoras doble hoja",
    slug: "rasuradoras-doble-hoja-100",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 11,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/rasuradoras.png",
    alt: "Paquete de rasuradoras de doble hoja desechables para estudio",
    shortDescription:
      "Rasuradoras de doble hoja en formato práctico para preparación de piel.",
    description:
      "Rasuradoras de doble hoja con peine de color verde. Presentación en caja de 100 unidades.",
    tags: ["material sanitario", "rasuradora", "preparación", "piel"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "depresores-madera-100",
    sku: "SAN-DEPRESORES-MADERA-100",
    name: "Depresores de madera",
    slug: "depresores-madera-100",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 1,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/depresores-madera.png",
    alt: "Depresores de lengua de madera embolsados para estudio",
    shortDescription:
      "Depresores de madera en bolsa de 100 unidades para aplicación y mezcla.",
    description:
      "Depresores de madera desechables, útiles para extender productos, mezclar o trabajar la zona con control. Formato caja de 100 unidades.",
    tags: ["material sanitario", "depresores", "madera", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "servilletas-absorbentes-30x30-100",
    sku: "SAN-SERVILLETAS-ABSORB-30X30-100",
    name: "Servilletas absorbentes 30x30",
    slug: "servilletas-absorbentes-30x30-100",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 6.15,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/servilletas-absorbentes.png",
    alt: "Servilletas absorbentes 30 por 30 centímetros en paquete",
    shortDescription:
      "Servilletas absorbentes 30x30 cm, paquete de 100 unidades.",
    description:
      "Servilletas de uso en sesión para absorber y trabajar con comodidad. Medidas 30x30 cm, presentación de 100 unidades.",
    tags: ["consumibles", "servilletas", "absorbente", "30x30"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "papel-camilla-polipropileno-precorte-1u",
    sku: "SAN-PAPEL-CAMILLA-PP-PRECORTE-N-1U",
    name: "Papel camilla polipropileno con precorte",
    slug: "papel-camilla-polipropileno-precorte-1u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 8,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/papel-camilla-polipropileno.png",
    alt: "Rollo de papel camilla de polipropileno con precorte para camilla",
    shortDescription:
      "Papel camilla de polipropileno con precorte. Disponible en negro y rosa.",
    description:
      "Rollo de papel camilla de polipropileno con precorte  cada 40cm para proteger la camilla y facilitar el cambio entre clientes. Muy agradable al tacto. Elige color al pedir.",
    tags: ["material sanitario", "papel camilla", "polipropileno", "precorte"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "Negro", price: 8, sku: "SAN-PAPEL-CAMILLA-PP-PRECORTE-N-1U" },
      { label: "Rosa", price: 8.5, sku: "SAN-PAPEL-CAMILLA-PP-PRECORTE-ROSA-1U" },
    ],
  },
  {
    id: "papel-camilla-resistente-sin-precorte-1u",
    sku: "SAN-PAPEL-CAMILLA-RESIST-SIN-PRECORTE-1U",
    name: "Papel camilla resistente sin precorte",
    slug: "papel-camilla-resistente-sin-precorte-1u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 8.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/papel-camilla-resistente.png",
    alt: "Rollo de papel camilla resistente sin precorte",
    shortDescription:
      "Papel camilla resistente sin precorte, formato de 1 unidad (rollo).",
    description:
      "Papel camilla de mayor resistencia sin precorte, pensado para sesiones largas y cambios puntuales.",
    tags: ["material sanitario", "camilla", "papel camilla", "resistente"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "papel-camilla-bicapa-precorte-1u",
    sku: "SAN-PAPEL-CAMILLA-BICAPA-PRECORTE-1U",
    name: "Papel camilla bicapa con precorte",
    slug: "papel-camilla-bicapa-precorte-1u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 7.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/papel-camilla-bicapa.png",
    alt: "Rollo de papel camilla bicapa con precorte",
    shortDescription:
      "Papel camilla bicapa con precorte para protección cómoda de la camilla.",
    description:
      "Rollo de papel camilla celulosa con precorte cada 50cm para una mayor comodidad.",
    tags: ["material sanitario", "camilla", "papel camilla", "bicapa", "precorte"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "film-transparente-rollo",
    sku: "SAN-FILM-TRANSPARENTE-30X300",
    name: "Film transparente",
    slug: "film-transparente-rollo",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 8,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/film-rosa.png",
    alt: "Rollo de film transparente para cubrir y proteger la piel o material",
    shortDescription:
      "Film transparente en rollo. Elige medida entre 30x300 y 45x300.",
    description:
      "Film transparente para uso en estudio. Disponible en distintas medidas de rollo, selecciona la opción al consultar por WhatsApp.",
    tags: ["consumibles", "film", "transparente", "barrera"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "30x300", price: 8, sku: "SAN-FILM-TRANSPARENTE-30X300" },
      { label: "45x300", price: 12, sku: "SAN-FILM-TRANSPARENTE-45X300" },
    ],
  },
  {
    id: "film-negro-50x120",
    sku: "SAN-FILM-NEGRO-50X120",
    name: "Film negro",
    slug: "film-negro-50x120",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 9,
    currency: "EUR",
    unit: "50x120",
    image: "images/productos/film-negro.png",
    alt: "Film negro para protección en estudio de tatuaje, formato 50x120",
    shortDescription:
      "Film negro en formato 50x120 para cubrir zona de trabajo o piel según uso.",
    description:
      "Film negro de uso profesional en estudio. Formato indicado 50x120 consulta detalles de presentación al pedir.",
    tags: ["consumibles", "film", "negro", "barrera"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "vendas-grip-12u",
    sku: "SAN-VENDAS-GRIP-NEGRO-12U",
    name: "Vendas grip",
    slug: "vendas-grip-12u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 12,
    currency: "EUR",
    unit: "12 unidades",
    image: "images/productos/vendas.png",
    alt: "Paquete de vendas grip para estudio de tatuaje",
    shortDescription:
      "Vendas grip en paquete de 12 unidades. Disponibles en negro y azul.",
    description:
      "Vendas de agarre para fijación muy resistentes. Presentación en caja de 12 unidades. Indica color al pedir por WhatsApp.",
    tags: ["material sanitario", "vendas", "grip", "fijación"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "Negro", price: 12, sku: "SAN-VENDAS-GRIP-NEGRO-12U" },
      { label: "Azul", price: 12, sku: "SAN-VENDAS-GRIP-AZUL-12U" },
    ],
    gallery: [
      "images/productos/vendas.png",
      "images/productos/venda-grip.png",
    ],
  },
  {
    id: "esparadrapo-tela-12u",
    sku: "SAN-ESPARADRAPO-TELA-12U",
    name: "Esparadrapo",
    slug: "esparadrapo-tela-12u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 5,
    currency: "EUR",
    unit: "12 unidades",
    image: "images/productos/esparadrapo1.png",
    alt: "Esparadrapo en rollo o caja para fijación en estudio",
    shortDescription:
      "Esparadrapo textil, paquete de 12 unidades para uso profesional.",
    description:
      "Esparadrapo textil para fijar apósitos o material con buena adherencia y retirada controlada. Formato de 12 unidades.",
    tags: ["consumibles", "esparadrapo", "tela", "fijación"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "servilletas-bicapa-50",
    sku: "SAN-SERVILLETAS-BICAPA-ROSA-50",
    name: "Servilletas bicapa",
    slug: "servilletas-bicapa-50",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 3.23,
    currency: "EUR",
    unit: "50 unidades",
    image: "images/productos/esparadrapo.png",
    alt: "Servilletas bicapa en paquete de 50 unidades para estudio",
    shortDescription:
      "Servilletas bicapa, paquete de 50 unidades. Disponibles en negro y rosa.",
    description:
      "Servilletas de estructura bicapa para absorción y manejo en sesión. Presentación de 50 unidades. Indica color al pedir por WhatsApp.",
    tags: ["consumibles", "servilletas", "bicapa", "negro", "rosa"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "Negro", price: 3.23, sku: "SAN-SERVILLETAS-BICAPA-NEGRO-50" },
      { label: "Rosa", price: 3.23, sku: "SAN-SERVILLETAS-BICAPA-ROSA-50" },
    ],
    gallery: [
      "images/productos/esparadrapo.png",
      "images/productos/servilletas-bicapa.png",
    ],
  },
  {
    id: "esterilizador-uv-1-pro",
    sku: "SAN-ESTERILIZADOR-UV-1-PRO",
    name: "Esterilizador UV 1 Pro",
    slug: "esterilizador-uv-1-pro",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "generico",
    price: 35,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/esterilizador.png",
    alt: "Esterilizador ultravioleta modelo 1 Pro para material de estudio",
    shortDescription:
      "Esterilizador UV 1 Pro para ciclo de desinfección de instrumental y accesorios compatibles.",
    description:
      "Equipo de esterilización por luz ultravioleta pensado para entornos profesionales. Consulta uso, compatibilidades y mantenimiento al pedir.",
    tags: ["desinfección", "esterilizador", "UV", "higiene", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "papel-secamanos-6x140m",
    sku: "SAN-PAPEL-SECA-MANOS-6X140M",
    name: "Papel secamanos",
    slug: "papel-secamanos-6x140m",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 28,
    currency: "EUR",
    unit: "6 unidades · 140m",
    image: "images/productos/papel-secamanos.png",
    alt: "Pack de papel secamanos para baño o aseo del estudio",
    shortDescription:
      "Bobina de papel secamanos, pack de 6 unidades.",
    description:
      "Bobina de papel secamanos, pack de 6 unidades, 140 metros por unidad. Ideal para alto tránsito.",
    tags: ["material sanitario", "papel", "secamanos", "higiene"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "sabanas-ajustables-5u",
    sku: "SAN-SABANAS-AJUSTABLES-BLANCO-5U",
    name: "Sábanas ajustables",
    slug: "sabanas-ajustables-5u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 4.25,
    currency: "EUR",
    unit: "5 unidades",
    image: "images/productos/sabana-ajustable.png",
    alt: "Sábanas ajustables desechables para camilla de tatuaje",
    shortDescription:
      "Sábanas ajustables en bolsa de 5 unidades. Disponibles en blanco y negro.",
    description:[
      "Sábanas con ajuste para camilla en formato bolsa de 5 unidades.",
      "Extremos con plástico ajustable, ideales para evitar manchas de tinta en la camilla. Elige color al pedir.",
    ],
    tags: ["material sanitario", "camilla", "sábanas", "desechables"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/sabana-ajustable.png",
      "images/productos/camilla.png",
    ],
    variants: [
      { label: "Blanco", price: 4.25, sku: "SAN-SABANAS-AJUSTABLES-BLANCO-5U" },
      { label: "Negro", price: 5.06, sku: "SAN-SABANAS-AJUSTABLES-NEGRO-5U" },
    ],
  },
  {
    id: "bolsas-esterilizadoras-200u",
    sku: "SAN-BOLSAS-ESTERIL-90X230-200U",
    name: "Bolsas esterilizadoras",
    slug: "bolsas-esterilizadoras-200u",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "generico",
    price: 18,
    currency: "EUR",
    unit: "200 unidades",
    image: "images/productos/bolsas-esterilizadoras.png",
    alt: "Bolsas para esterilización en autoclave, caja de 200 unidades",
    shortDescription:
      "Bolsas esterilizadoras en caja de 200 unidades. Varios tamaños según medida.",
    description:
      "Bolsas autoadhesivas para la esterilización, resistentes y de alta calidad. Caja de 200 unidades. Disponible en diferentes medidas. Consultar stock y precio por WhatsApp.",
    tags: ["esterilizacion", "bolsas", "autoclave"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "90x230", price: 18, sku: "SAN-BOLSAS-ESTERIL-90X230-200U" },
      { label: "130x254", price: 25, sku: "SAN-BOLSAS-ESTERIL-130X254-200U" },
      { label: "200x330", price: 44.3, sku: "SAN-BOLSAS-ESTERIL-200X330-200U" },
    ],
  },
  {
    id: "desinfectante-piel-liquido-150ml",
    sku: "SAN-DESINFECTANTE-PIEL-LIQUIDO-150ML",
    name: "Desinfectante de piel líquido EFFIGERM",
    slug: "desinfectante-piel-liquido-150ml",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "effigerm",
    price: 9.5,
    currency: "EUR",
    unit: "150ml",
    image: "images/productos/effigerm-prot.png",
    alt: "Frasco de desinfectante de piel en formato líquido 150 ml",
    shortDescription:
      "Desinfectante de piel en formato líquido, 150 ml, para preparación en estudio.",
    description:[
      "Effigerm Wound Care es una solución con ácido hipocloroso al 0,016% (160 ppm), el desinfectante más potente y natural que existe.",
      "Limpia, hidrata y acelera la cicatrización sin dolor ni escozor. Previene infecciones, elimina malos olores y protege frente a biofilms.",
    ],
    tags: ["esterilizacion", "piel", "líquido", "preparación"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/desinfectante.png",
      "images/productos/desinfectante2.png",
    ],
  },
  {
    id: "desinfectante-piel-hydrogel-150ml",
    sku: "SAN-DESINFECTANTE-PIEL-HYDROGEL-150ML",
    name: "Desinfectante de piel Hydrogel Effigerm",
    slug: "desinfectante-piel-hydrogel-150ml",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "effigerm",
    price: 10,
    currency: "EUR",
    unit: "150ml",
    image: "images/productos/hydrogel.png",
    alt: "Frasco de desinfectante de piel tipo hydrogel 150 ml",
    shortDescription:
      "Desinfectante de piel en gel acuoso (hydrogel), 150 ml, cómodo de extender.",
    description:[
      "Effigerm Wound Care es una solución con ácido hipocloroso al 0,016% (160 ppm), el desinfectante más potente y natural que existe. Formato hydrogel para aplicación uniforme sobre la piel en fase de preparación.",
      "Limpia, hidrata y acelera la cicatrización sin dolor ni escozor. Previene infecciones, elimina malos olores y protege frente a biofilms. ",
    ],
    tags: ["desinfección", "piel", "hydrogel", "gel", "preparación"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "capsulas-con-base-100u",
    sku: "SAN-CAPSULAS-BASE-10MM-100U",
    name: "Cápsulas con base",
    slug: "capsulas-con-base-100u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 3.5,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/capsulas-1.png",
    alt: "Cápsulas con base para pigmentos en formato bolsa de 100 unidades",
    shortDescription:
      "Cápsulas con base en bolsa de 100 unidades. Varios diámetros según medida.",
    description:
      "Cápsulas desechables con base estable para trabajar cómodamente en sesión. Elige diámetro (10, 13 o 17 mm) al consultar por WhatsApp.",
    tags: ["consumibles", "cápsulas", "tinta", "pigmento", "estudio"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "10mm", price: 3.5, sku: "SAN-CAPSULAS-BASE-10MM-100U" },
      { label: "13mm", price: 3.75, sku: "SAN-CAPSULAS-BASE-13MM-100U" },
      { label: "17mm", price: 4, sku: "SAN-CAPSULAS-BASE-17MM-100U" },
    ],
  },
  {
    id: "contenedor-agujas-2l",
    sku: "SAN-CONTENEDOR-AGUJAS-2L",
    name: "Contenedor de agujas",
    slug: "contenedor-agujas-2l",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "generico",
    price: 4,
    currency: "EUR",
    unit: "2L",
    image: "images/productos/cubo.png",
    alt: "Contenedor rígido para residuos punzantes de 2 litros",
    shortDescription:
      "Contenedor para agujas y material punzante, capacidad 2 litros.",
    description:
      "Recipiente de seguridad para el depósito de agujas y residuos cortopunzantes según protocolo del estudio.",
    tags: ["esterilizacion", "contenedor", "residuos", "desinfección"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "botella-boxer-500ml",
    sku: "SAN-BOTELLA-BOXER-500ML",
    name: "Botella bóxer",
    slug: "botella-boxer-500ml",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 3.5,
    currency: "EUR",
    unit: "500ml",
    image: "images/productos/botella.png",
    alt: "Botella tipo bóxer de 500 ml para pulverizar o aplicar líquido en sesión",
    shortDescription:
      "Botella bóxer de 500 ml, formato habitual para líquidos de trabajo en estudio.",
    description:
      "Botella con rosca y boquilla tipo bóxer para dispensar líquidos de forma controlada. Uso profesional en tatuaje. Consulta compatibilidad de contenido.",
    tags: ["material sanitario", "botella", "bóxer", "pulverizar", "estudio"],
    featured: false,
    offer: false,
    active: true,
    variants: [
      { label: "250ml", price: 3.5, sku: "SAN-BOTELLA-BOXER-250ML" },
    ],
    gallery: [
      "images/productos/botella.png",
      "images/productos/botella2.webp",
    ],
  },
  {
    id: "mangas-desechables-100u",
    sku: "SAN-MANGAS-DESECHABLES-100U",
    name: "Mangas desechables",
    slug: "mangas-desechables-100u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 7.03,
    currency: "EUR",
    unit: "100 unidades",
    image: "images/productos/mangas1.png",
    alt: "Mangas desechables para brazos en caja de 100 unidades",
    shortDescription:
      "Mangas desechables para brazos, bolsa de 100 unidades.",
    description:[
      "Protección desechable para antebrazo durante la sesión. Material ligero y práctico para mantener la higiene de la zona de trabajo.",
      "Presentación en caja de 100 unidades.",
    ],
    tags: ["material sanitario", "higiene", "mangas", "desechables", "brazo"],
    featured: false,
    offer: false,
    active: true,
    gallery: [
      "images/productos/mangas1.png",
      "images/productos/mangas2.png",
    ],
  },
  {
    id: "alcohol-96-500ml",
    sku: "SAN-ALCOHOL-96-500ML",
    name: "Alcohol VAZA 96%",
    slug: "alcohol-96-500ml",
    category: "esterilizacion",
    categoryLabel: "Esterilización",
    brand: "generico",
    price: 3,
    currency: "EUR",
    unit: "500ml",
    image: "images/productos/alcohol-96.png",
    alt: "Envase de alcohol etílico al 96% en presentación 500 ml",
    shortDescription:
      "Alcohol etílico al 96 %, frasco de 500 ml para limpieza y desinfección en estudio.",
    description:
      "Antiséptico y bactericida para piel sana, destinado al uso sanitario, clínico y hospitalario, con actividad desinfectante, prepara la piel para trabajar en ella. Solo uso externo.",
    tags: ["esterilizacion", "alcohol", "limpieza", "estudio"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "vasos-plastico-negro-50u",
    sku: "SAN-VASOS-PLASTICO-NEGRO-50U",
    name: "Vasos de plástico negros",
    slug: "vasos-plastico-negro-50u",
    category: "material-sanitario",
    categoryLabel: "Material sanitario",
    brand: "generico",
    price: 2.44,
    currency: "EUR",
    unit: "50 unidades",
    image: "images/productos/vasos-plastico (2).png",
    alt: "Vasos de plástico negros desechables en paquete de 50 unidades",
    shortDescription:
      "Vasos de plástico negro, paquete de 50 unidades para mezcla o pigmento.",
    description:
      "Vasos desechables de color negro, útiles para diluir tinta o organizar pigmentos en sesión. Presentación en caja de 50 unidades.",
    tags: ["consumibles", "vasos", "plástico", "negro", "pigmento"],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-1rl",
    sku: "SAN-AGUJAS-RL-1RL",
    name: "Aguja Tsunami 1RL ",
    slug: "aguja-rl-1rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 1RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 3RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "060RL", price: 26, sku: "SAN-AGUJAS-RL-1RL" },
      { label: "080RL", price: 26, sku: "SAN-AGUJAS-RL-1RL" },
      { label: "120RL", price: 26, sku: "SAN-AGUJAS-RL-1RL" },
      { label: "1001RL", price: 26, sku: "SAN-AGUJAS-RL-1RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-3rl",
    sku: "SAN-AGUJAS-RL-3RL",
    name: "Aguja Tsunami 3RL ",
    slug: "aguja-rl-3rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 3RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 3RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "060RL", price: 26, sku: "SAN-AGUJAS-RL-3RL" },
      { label: "080RL", price: 26, sku: "SAN-AGUJAS-RL-3RL" },
      { label: "100RL", price: 26, sku: "SAN-AGUJAS-RL-3RL" },
      { label: "120RL", price: 26, sku: "SAN-AGUJAS-RL-3RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-5rl",
    sku: "SAN-AGUJAS-RL-5RL",
    name: "Aguja Tsunami 5RL ",
    slug: "aguja-rl-5rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 5RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 5RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "080RL", price: 27, sku: "SAN-AGUJAS-RL-5RL" },
      { label: "100RL", price: 27, sku: "SAN-AGUJAS-RL-5RL" },
      { label: "120RL", price: 27, sku: "SAN-AGUJAS-RL-5RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-7rl",
    sku: "SAN-AGUJAS-RL-7RL",
    name: "Aguja Tsunami 7RL ",
    slug: "aguja-rl-7rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 28,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 7RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 7RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "080RL", price: 28, sku: "SAN-AGUJAS-RL-7RL" },
      { label: "100RL", price: 28, sku: "SAN-AGUJAS-RL-7RL" },
      { label: "120RL", price: 28, sku: "SAN-AGUJAS-RL-7RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-9rl",
    sku: "SAN-AGUJAS-RL-9RL",
    name: "Aguja Tsunami 9RL ",
    slug: "aguja-rl-9rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 9RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 9RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "080RL", price: 29, sku: "SAN-AGUJAS-RL-9RL" },
      { label: "100RL", price: 29, sku: "SAN-AGUJAS-RL-9RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-11rl",
    sku: "SAN-AGUJAS-RL-11RL",
    name: "Aguja Tsunami 11RL ",
    slug: "aguja-rl-11rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 11RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 11RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "10RL", price: 29, sku: "SAN-AGUJAS-RL-11RL" },
      { label: "12RL", price: 29, sku: "SAN-AGUJAS-RL-11RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-14rl",
    sku: "SAN-AGUJAS-RL-14RL",
    name: "Aguja Tsunami 14RL ",
    slug: "aguja-rl-14rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 30,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 14RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 14RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "10RL", price: 30, sku: "SAN-AGUJAS-RL-14RL" },
      { label: "12RL", price: 30, sku: "SAN-AGUJAS-RL-14RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rl-18rl",
    sku: "SAN-AGUJAS-RL-18RL",
    name: "Aguja Tsunami 18RL ",
    slug: "aguja-rl-18rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rl",
    price: 35,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rl.jpeg",
    alt: "Aguja RL 18RL para tatuaje",
    shortDescription:
      "Aguja Tsunami 18RL para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "10RL", price: 35, sku: "SAN-AGUJAS-RL-18RL" },
      { label: "12RL", price: 35, sku: "SAN-AGUJAS-RL-18RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"aguja-rs-3rs",
    sku: "SAN-AGUJAS-RS-3RS",
    name: "Aguja Tsunami 3RS ",
    slug: "aguja-rs-3rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 3RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 3RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1203RS", price: 26, sku: "SAN-AGUJAS-RS-3RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rs-5rs",
    sku: "SAN-AGUJAS-RS-5RS",
    name: "Aguja Tsunami 5RS ",
    slug: "aguja-rs-5rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 5RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 5RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1005RS", price: 26, sku: "SAN-AGUJAS-RS-5RS" },
      { label: "1205RS", price: 26, sku: "SAN-AGUJAS-RS-5RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rs-7rs",
    sku: "SAN-AGUJAS-RS-7RS",
    name: "Aguja Tsunami 7RS ",
    slug: "aguja-rs-7rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 7RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 7RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1007RS", price: 26, sku: "SAN-AGUJAS-RS-7RS" },
      { label: "1207RS", price: 26, sku: "SAN-AGUJAS-RS-7RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rs-9rs",
    sku: "SAN-AGUJAS-RS-9RS",
    name: "Aguja Tsunami 9RS ",
    slug: "aguja-rs-9rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 9RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 9RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1009RS", price: 27, sku: "SAN-AGUJAS-RS-9RS" },
      { label: "1209RS", price: 27, sku: "SAN-AGUJAS-RS-9RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rs-11rs",
    sku: "SAN-AGUJAS-RS-11RS",
    name: "Aguja Tsunami 11RS ",
    slug: "aguja-rs-11rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 11RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 11RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1011RS", price: 27, sku: "SAN-AGUJAS-RS-11RS" },
      { label: "1211RS", price: 27, sku: "SAN-AGUJAS-RS-11RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rs-14rs",
    sku: "SAN-AGUJAS-RS-14RS",
    name: "Aguja Tsunami 14RS ",
    slug: "aguja-rs-14rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 28,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 14RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 14RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1014RS", price: 28, sku: "SAN-AGUJAS-RS-14RS" },
      { label: "1214RS", price: 28, sku: "SAN-AGUJAS-RS-14RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-rs-18rs",
    sku: "SAN-AGUJAS-RS-18RS",
    name: "Aguja Tsunami 18RS ",
    slug: "aguja-rs-18rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "rs",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-rs.png",
    alt: "Aguja RS 18RS para tatuaje",
    shortDescription:
      "Aguja Tsunami 18RS para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1018RS", price: 29, sku: "SAN-AGUJAS-RS-18RS" },
      { label: "1218RS", price: 29, sku: "SAN-AGUJAS-RS-18RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-5m",
    sku: "SAN-AGUJAS-M-5M",
    name: "Aguja Tsunami 5M ",
    slug: "aguja-m-5m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 5M para tatuaje",
    shortDescription:
      "Aguja Tsunami 5M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1005M", price: 26, sku: "SAN-AGUJAS-M-5M" },
      { label: "1205M", price: 26, sku: "SAN-AGUJAS-M-5M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-7m",
    sku: "SAN-AGUJAS-M-7M",
    name: "Aguja Tsunami 7M ",
    slug: "aguja-m-7m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 7M para tatuaje",
    shortDescription:
      "Aguja Tsunami 7M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1007M", price: 26, sku: "SAN-AGUJAS-M-7M" },
      { label: "1207M", price: 26, sku: "SAN-AGUJAS-M-7M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-9m",
    sku: "SAN-AGUJAS-M-9M",
    name: "Aguja Tsunami 9M ",
    slug: "aguja-m-9m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 9M para tatuaje",
    shortDescription:
      "Aguja Tsunami 9M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1009M", price: 27, sku: "SAN-AGUJAS-M-9M" },
      { label: "1209M", price: 27, sku: "SAN-AGUJAS-M-9M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-11m",
    sku: "SAN-AGUJAS-M-11M",
    name: "Aguja Tsunami 11M ",
    slug: "aguja-m-11m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 11M para tatuaje",
    shortDescription:
      "Aguja Tsunami 11M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1011M", price: 27, sku: "SAN-AGUJAS-M-11M" },
      { label: "1211M", price: 27, sku: "SAN-AGUJAS-M-11M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-13m",
    sku: "SAN-AGUJAS-M-13M",
    name: "Aguja Tsunami 13M ",
    slug: "aguja-m-13m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 28,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 13M para tatuaje",
    shortDescription:
      "Aguja Tsunami 13M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1013M", price: 28, sku: "SAN-AGUJAS-M-13M" },
      { label: "1213M", price: 28, sku: "SAN-AGUJAS-M-13M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-15m",
    sku: "SAN-AGUJAS-M-15M",
    name: "Aguja Tsunami 15M ",
    slug: "aguja-m-15m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 15M para tatuaje",
    shortDescription:
      "Aguja Tsunami 15M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1015M", price: 29, sku: "SAN-AGUJAS-M-15M" },
      { label: "1215M", price: 29, sku: "SAN-AGUJAS-M-15M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-17m",
    sku: "SAN-AGUJAS-M-17M",
    name: "Aguja Tsunami 17M ",
    slug: "aguja-m-17m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 30,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 17M para tatuaje",
    shortDescription:
      "Aguja Tsunami 17M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1017M", price: 30, sku: "SAN-AGUJAS-M-17M" },
      { label: "1217M", price: 30, sku: "SAN-AGUJAS-M-17M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-23m",
    sku: "SAN-AGUJAS-M-23M",
    name: "Aguja Tsunami 23M ",
    slug: "aguja-m-23m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 31,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 23M para tatuaje",
    shortDescription:
      "Aguja Tsunami 23M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1023M", price: 31, sku: "SAN-AGUJAS-M-23M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-25m",
    sku: "SAN-AGUJAS-M-25M",
    name: "Aguja Tsunami 25M ",
    slug: "aguja-m-25m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 32,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 25M para tatuaje",
    shortDescription:
      "Aguja Tsunami 25M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1225M", price: 32, sku: "SAN-AGUJAS-M-25M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-m-27m",
    sku: "SAN-AGUJAS-M-27M",
    name: "Aguja Tsunami 27M ",
    slug: "aguja-m-27m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "m",
    price: 32,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-m.png",
    alt: "Aguja M 27M para tatuaje",
    shortDescription:
      "Aguja Tsunami 27M para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      
      { label: "1027M", price: 32, sku: "SAN-AGUJAS-M-27M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id:"aguja-cm-5cm",
    sku: "SAN-AGUJAS-CM-5CM",
    name: "Aguja Tsunami 5CM ",
    slug: "aguja-cm-5cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 5CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 5CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1005CM", price: 26, sku: "SAN-AGUJAS-CM-5CM" },
      { label: "1205CM", price: 26, sku: "SAN-AGUJAS-CM-5CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-7cm",
    sku: "SAN-AGUJAS-CM-7CM",
    name: "Aguja Tsunami 7CM ",
    slug: "aguja-cm-7cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 7CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 7CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1007CM", price: 26, sku: "SAN-AGUJAS-CM-7CM" },
      { label: "1207CM", price: 26, sku: "SAN-AGUJAS-CM-7CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-9cm",
    sku: "SAN-AGUJAS-CM-9CM",
    name: "Aguja Tsunami 9CM ",
    slug: "aguja-cm-9cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 9CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 9CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "0809CM", price: 27, sku: "SAN-AGUJAS-CM-9CM" },
      { label: "1009CM", price: 27, sku: "SAN-AGUJAS-CM-9CM" },
      { label: "1209CM", price: 27, sku: "SAN-AGUJAS-CM-9CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-11cm",
    sku: "SAN-AGUJAS-CM-11CM",
    name: "Aguja Tsunami 11CM ",
    slug: "aguja-cm-11cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 11CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 11CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "0811CM", price: 27, sku: "SAN-AGUJAS-CM-11CM" },
      { label: "1011CM", price: 27, sku: "SAN-AGUJAS-CM-11CM" },
      { label: "1211CM", price: 27, sku: "SAN-AGUJAS-CM-11CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-13cm",
    sku: "SAN-AGUJAS-CM-13CM",
    name: "Aguja Tsunami 13CM ",
    slug: "aguja-cm-13cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 28,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 13CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 13CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "0813CM", price: 28, sku: "SAN-AGUJAS-CM-13CM" },
      { label: "1013CM", price: 28, sku: "SAN-AGUJAS-CM-13CM" },
      { label: "1213CM", price: 28, sku: "SAN-AGUJAS-CM-13CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-15cm",
    sku: "SAN-AGUJAS-CM-15CM",
    name: "Aguja Tsunami 15CM ",
    slug: "aguja-cm-15cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 15CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 15CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "0815CM", price: 29, sku: "SAN-AGUJAS-CM-15CM" },
      { label: "1015CM", price: 29, sku: "SAN-AGUJAS-CM-15CM" },
      { label: "1215CM", price: 29, sku: "SAN-AGUJAS-CM-15CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-17cm",
    sku: "SAN-AGUJAS-CM-17CM",
    name: "Aguja Tsunami 17CM ",
    slug: "aguja-cm-17cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 17CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 17CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1017CM", price: 29, sku: "SAN-AGUJAS-CM-17CM" },
      { label: "1217CM", price: 29, sku: "SAN-AGUJAS-CM-17CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-19cm",
    sku: "SAN-AGUJAS-CM-19CM",
    name: "Aguja Tsunami 19CM ",
    slug: "aguja-cm-19cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 29,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 19CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 19CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1019CM", price: 29, sku: "SAN-AGUJAS-CM-19CM" },
      { label: "1219CM", price: 29, sku: "SAN-AGUJAS-CM-19CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-23cm",
    sku: "SAN-AGUJAS-CM-23CM",
    name: "Aguja Tsunami 23CM ",
    slug: "aguja-cm-23cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 31,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 23CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 23CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1023CM", price: 31, sku: "SAN-AGUJAS-CM-23CM" },
      { label: "1223CM", price: 31, sku: "SAN-AGUJAS-CM-23CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-25cm",
    sku: "SAN-AGUJAS-CM-25CM",
    name: "Aguja Tsunami 25CM ",
    slug: "aguja-cm-25cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 31,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 25CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 25CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1025CM", price: 31, sku: "SAN-AGUJAS-CM-25CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "aguja-cm-27cm",
    sku: "SAN-AGUJAS-CM-27CM",
    name: "Aguja Tsunami 27CM ",
    slug: "aguja-cm-27cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "tsunami",
    needleLine: "cm",
    price: 33,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/aguja-cm.png",
    alt: "Aguja CM 27CM para tatuaje",
    shortDescription:
      "Aguja Tsunami 27CM para tatuaje.",
    description:[
      "Cartucho con membrana, fabricado con los mejores materiales. Cuidando al máximo cada detalle en su fabricación, totalmente configuradas por profesionales. Incorporan membrana de seguridad antifluidos.",
      "Envasadas individualmente y esterilizadas con E.O gas.",
      "Fabricadas con indicador de color para difereniciar fácilmente las agujas que estás utilizando.",
      "Fabricadas con acero 316L y plástico de grado médico con transparencia para mejorar la visión."
    ],
    tags: ["agujas", "tatuaje", "tsunami"],
    variants: [
      { label: "1027CM", price: 33, sku: "SAN-AGUJAS-CM-27CM" },
      { label: "1227CM", price: 33, sku: "SAN-AGUJAS-CM-27CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-1rl",
    sku: "SAN-SHAPU-RL-1RL",
    name: "Aguja Shapu 1RL",
    slug: "shapu-rl-1rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 22,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 1RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "0801RL", price: 22, sku: "SAN-SHAPU-RL-1RL-0801RL" },
      { label: "1001RL", price: 22, sku: "SAN-SHAPU-RL-1RL-1001RL" },
      { label: "1201RL", price: 22, sku: "SAN-SHAPU-RL-1RL-1201RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-3rl",
    sku: "SAN-SHAPU-RL-3RL",
    name: "Aguja Shapu 3RL",
    slug: "shapu-rl-3rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 22,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 3RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "0803RL", price: 22, sku: "SAN-SHAPU-RL-3RL-0803RL" },
      { label: "1003RL", price: 22, sku: "SAN-SHAPU-RL-3RL-1003RL" },
      { label: "1203RL", price: 22, sku: "SAN-SHAPU-RL-3RL-1203RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-5rl",
    sku: "SAN-SHAPU-RL-5RL",
    name: "Aguja Shapu 5RL",
    slug: "shapu-rl-5rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 23,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 5RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "0805RL", price: 23, sku: "SAN-SHAPU-RL-5RL-0805RL" },
      { label: "1005RL", price: 23, sku: "SAN-SHAPU-RL-5RL-1005RL" },
      { label: "1205RL", price: 23, sku: "SAN-SHAPU-RL-5RL-1205RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-7rl",
    sku: "SAN-SHAPU-RL-7RL",
    name: "Aguja Shapu 7RL",
    slug: "shapu-rl-7rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 24,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 7RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "0807RL", price: 24, sku: "SAN-SHAPU-RL-7RL-0807RL" },
      { label: "1007RL", price: 24, sku: "SAN-SHAPU-RL-7RL-1007RL" },
      { label: "1207RL", price: 24, sku: "SAN-SHAPU-RL-7RL-1207RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-9rl",
    sku: "SAN-SHAPU-RL-9RL",
    name: "Aguja Shapu 9RL",
    slug: "shapu-rl-9rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 25,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 9RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "0809RL", price: 25, sku: "SAN-SHAPU-RL-9RL-0809RL" },
      { label: "1009RL", price: 25, sku: "SAN-SHAPU-RL-9RL-1009RL" },
      { label: "1209RL", price: 25, sku: "SAN-SHAPU-RL-9RL-1209RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-11rl",
    sku: "SAN-SHAPU-RL-11RL",
    name: "Aguja Shapu 11RL",
    slug: "shapu-rl-11rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 11RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "1011RL", price: 26, sku: "SAN-SHAPU-RL-11RL-1011RL" },
      { label: "1211RL", price: 26, sku: "SAN-SHAPU-RL-11RL-1211RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-14rl",
    sku: "SAN-SHAPU-RL-14RL",
    name: "Aguja Shapu 14RL",
    slug: "shapu-rl-14rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 14RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "1014RL", price: 27, sku: "SAN-SHAPU-RL-14RL-1014RL" },
      { label: "1214RL", price: 27, sku: "SAN-SHAPU-RL-14RL-1214RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rl-18rl",
    sku: "SAN-SHAPU-RL-18RL",
    name: "Aguja Shapu 18RL",
    slug: "shapu-rl-18rl",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rl",
    price: 28.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rl.JPG",
    alt: "Aguja Shapu 18RL para tatuaje",
    shortDescription: "Aguja Shapu línea RL para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rl"],
    variants: [
      { label: "1018RL", price: 28.5, sku: "SAN-SHAPU-RL-18RL-1018RL" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-3rs",
    sku: "SAN-SHAPU-RS-3RS",
    name: "Aguja Shapu 3RS",
    slug: "shapu-rs-3rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 22,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 3RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1203RS", price: 22, sku: "SAN-SHAPU-RS-3RS-1203RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-5rs",
    sku: "SAN-SHAPU-RS-5RS",
    name: "Aguja Shapu 5RS",
    slug: "shapu-rs-5rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 23,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 5RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1005RS", price: 23, sku: "SAN-SHAPU-RS-5RS-1005RS" },
      { label: "1205RS", price: 23, sku: "SAN-SHAPU-RS-5RS-1205RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-7rs",
    sku: "SAN-SHAPU-RS-7RS",
    name: "Aguja Shapu 7RS",
    slug: "shapu-rs-7rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 24,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 7RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1007RS", price: 24, sku: "SAN-SHAPU-RS-7RS-1007RS" },
      { label: "1207RS", price: 24, sku: "SAN-SHAPU-RS-7RS-1207RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-9rs",
    sku: "SAN-SHAPU-RS-9RS",
    name: "Aguja Shapu 9RS",
    slug: "shapu-rs-9rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 25,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 9RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1009RS", price: 25, sku: "SAN-SHAPU-RS-9RS-1009RS" },
      { label: "1209RS", price: 25, sku: "SAN-SHAPU-RS-9RS-1209RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-11rs",
    sku: "SAN-SHAPU-RS-11RS",
    name: "Aguja Shapu 11RS",
    slug: "shapu-rs-11rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 11RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1211RS", price: 26, sku: "SAN-SHAPU-RS-11RS-1211RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-14rs",
    sku: "SAN-SHAPU-RS-14RS",
    name: "Aguja Shapu 14RS",
    slug: "shapu-rs-14rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 14RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1214RS", price: 27, sku: "SAN-SHAPU-RS-14RS-1214RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rs-18rs",
    sku: "SAN-SHAPU-RS-18RS",
    name: "Aguja Shapu 18RS",
    slug: "shapu-rs-18rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rs",
    price: 28.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rs.JPG",
    alt: "Aguja Shapu 18RS para tatuaje",
    shortDescription: "Aguja Shapu línea RS para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rs"],
    variants: [
      { label: "1218RS", price: 28.5, sku: "SAN-SHAPU-RS-18RS-1218RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-5m",
    sku: "SAN-SHAPU-M-5M",
    name: "Aguja Shapu 5M",
    slug: "shapu-m-5m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 23,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 5M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1005M", price: 23, sku: "SAN-SHAPU-M-5M-1005M" },
      { label: "1205M", price: 23, sku: "SAN-SHAPU-M-5M-1205M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-7m",
    sku: "SAN-SHAPU-M-7M",
    name: "Aguja Shapu 7M",
    slug: "shapu-m-7m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 24,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 7M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1007M", price: 24, sku: "SAN-SHAPU-M-7M-1007M" },
      { label: "1207M", price: 24, sku: "SAN-SHAPU-M-7M-1207M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-9m",
    sku: "SAN-SHAPU-M-9M",
    name: "Aguja Shapu 9M",
    slug: "shapu-m-9m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 25,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 9M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1009M", price: 25, sku: "SAN-SHAPU-M-9M-1009M" },
      { label: "1209M", price: 25, sku: "SAN-SHAPU-M-9M-1209M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-11m",
    sku: "SAN-SHAPU-M-11M",
    name: "Aguja Shapu 11M",
    slug: "shapu-m-11m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 11M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1011M", price: 26, sku: "SAN-SHAPU-M-11M-1011M" },
      { label: "1211M", price: 26, sku: "SAN-SHAPU-M-11M-1211M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-13m",
    sku: "SAN-SHAPU-M-13M",
    name: "Aguja Shapu 13M",
    slug: "shapu-m-13m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 13M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1013M", price: 27, sku: "SAN-SHAPU-M-13M-1013M" },
      { label: "1213M", price: 27, sku: "SAN-SHAPU-M-13M-1213M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-15m",
    sku: "SAN-SHAPU-M-15M",
    name: "Aguja Shapu 15M",
    slug: "shapu-m-15m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 28,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 15M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1015M", price: 28, sku: "SAN-SHAPU-M-15M-1015M" },
      { label: "1215M", price: 28, sku: "SAN-SHAPU-M-15M-1215M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-m-23m",
    sku: "SAN-SHAPU-M-23M",
    name: "Aguja Shapu 23M",
    slug: "shapu-m-23m",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "m",
    price: 32.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-m.JPG",
    alt: "Aguja Shapu 23M para tatuaje",
    shortDescription: "Aguja Shapu Magnum M para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "m"],
    variants: [
      { label: "1023M", price: 32.5, sku: "SAN-SHAPU-M-23M-1023M" },
      { label: "1223M", price: 32.5, sku: "SAN-SHAPU-M-23M-1223M" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-5rm",
    sku: "SAN-SHAPU-RM-5RM",
    name: "Aguja Shapu 5RM",
    slug: "shapu-rm-5rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 23,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 5RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1005RM", price: 23, sku: "SAN-SHAPU-RM-5RM-1005RM" },
      { label: "1205RM", price: 23, sku: "SAN-SHAPU-RM-5RM-1205RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-7rm",
    sku: "SAN-SHAPU-RM-7RM",
    name: "Aguja Shapu 7RM",
    slug: "shapu-rm-7rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 24,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 7RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1007RM", price: 24, sku: "SAN-SHAPU-RM-7RM-1007RM" },
      { label: "1207RM", price: 24, sku: "SAN-SHAPU-RM-7RM-1207RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-9rm",
    sku: "SAN-SHAPU-RM-9RM",
    name: "Aguja Shapu 9RM",
    slug: "shapu-rm-9rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 25,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 9RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1009RM", price: 25, sku: "SAN-SHAPU-RM-9RM-1009RM" },
      { label: "1209RM", price: 25, sku: "SAN-SHAPU-RM-9RM-1209RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-11rm",
    sku: "SAN-SHAPU-RM-11RM",
    name: "Aguja Shapu 11RM",
    slug: "shapu-rm-11rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 26,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 11RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1011RM", price: 26, sku: "SAN-SHAPU-RM-11RM-1011RM" },
      { label: "1211RM", price: 26, sku: "SAN-SHAPU-RM-11RM-1211RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-13rm",
    sku: "SAN-SHAPU-RM-13RM",
    name: "Aguja Shapu 13RM",
    slug: "shapu-rm-13rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 27,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 13RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1013RM", price: 27, sku: "SAN-SHAPU-RM-13RM-1013RM" },
      { label: "1213RM", price: 27, sku: "SAN-SHAPU-RM-13RM-1213RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-15rm",
    sku: "SAN-SHAPU-RM-15RM",
    name: "Aguja Shapu 15RM",
    slug: "shapu-rm-15rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 28,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 15RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1015RM", price: 28, sku: "SAN-SHAPU-RM-15RM-1015RM" },
      { label: "1215RM", price: 28, sku: "SAN-SHAPU-RM-15RM-1215RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-19rm",
    sku: "SAN-SHAPU-RM-19RM",
    name: "Aguja Shapu 19RM",
    slug: "shapu-rm-19rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 30.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 19RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1019RM", price: 30.5, sku: "SAN-SHAPU-RM-19RM-1019RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-21rm",
    sku: "SAN-SHAPU-RM-21RM",
    name: "Aguja Shapu 21RM",
    slug: "shapu-rm-21rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 31.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 21RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1021RM", price: 31.5, sku: "SAN-SHAPU-RM-21RM-1021RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-23rm",
    sku: "SAN-SHAPU-RM-23RM",
    name: "Aguja Shapu 23RM",
    slug: "shapu-rm-23rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 32.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 23RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1023RM", price: 32.5, sku: "SAN-SHAPU-RM-23RM-1023RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "shapu-rm-27rm",
    sku: "SAN-SHAPU-RM-27RM",
    name: "Aguja Shapu 27RM",
    slug: "shapu-rm-27rm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "shapu",
    needleLine: "rm",
    price: 33.5,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/productos/shapu-rm.JPG",
    alt: "Aguja Shapu 27RM para tatuaje",
    shortDescription: "Aguja Shapu Round Magnum RM para tatuaje.",
    description: [
      "Shapu combina precisión japonesa y materiales de alta calidad para ofrecer agujas fiables y estables.",
      "Utilizamos un método de soldadura de baja temperatura y libre de plomo que protege la integridad del acero, evita que se desafile la punta y garantiza una unión firme y duradera.",
      "Las agujas soldadas están perfectamente uniformes y fabricadas con acero endurecido para lograr mayor resistencia y una larga vida útil.",
      "Gracias al estricto proceso de fabricación, cada aguja encaja de forma exacta en su cartucho, reduciendo al mínimo la vibración y aportando control en cada trazo.",
      "Shapu representa un equilibrio entre tecnología, fiabilidad y la dedicación por crear herramientas que acompañen al artista en cada detalle.",
    ],
    tags: ["agujas", "tatuaje", "shapu", "rm"],
    variants: [
      { label: "1027RM", price: 33.5, sku: "SAN-SHAPU-RM-27RM-1027RM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "black-mambo-rlt-3rlt",
    sku: "SAN-BLACK-MAMBO-RLT-3RLT",
    name: "Aguja Black Mambo 3RLT",
    slug: "black-mambo-rlt-3rlt",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "black-mambo",
    needleLine: "rlt",
    price: 25,
    currency: "EUR",
    unit: "20 unidades",
    image: "images/productos/black-aguja.png",
    alt: "Aguja Black Mambo 3RLT para tatuaje",
    shortDescription: "Aguja Black Mambo línea RLT para tatuaje.",
    description: [
      "Las agujas de acero quirúrgico de BLACK MAMBO están creadas con los más altos estándares de calidad.",
      "El mejor acero y la aleación perfecta, creado con la más alta precisión en Alemania.",
      "El apriete en la agrupación de las mismas dará una alta solidez en cualquier tipo de trabajo.",
      "El desgaste y el despunte no serán un problema incluso en grandes trabajos.",
      "Nuestro estricto control de calidad al inicio y fin de serie mediante un palpado de precisión a la micra garantiza a nuestras puntas un ajuste y calibrado perfecto.",
      "Esto hace que las agujas BLACK MAMBO PREMIUM cumplan con los más altos estándares de calidad.",
    ],
    tags: ["agujas", "tatuaje", "black mambo", "rlt"],
    variants: [
      { label: "0803RLT", price: 25, sku: "SAN-BLACK-MAMBO-RLT-3RLT-0803RLT" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "black-mambo-rlt-9rlt",
    sku: "SAN-BLACK-MAMBO-RLT-9RLT",
    name: "Aguja Black Mambo 9RLT",
    slug: "black-mambo-rlt-9rlt",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "black-mambo",
    needleLine: "rlt",
    price: 27,
    currency: "EUR",
    unit: "20 unidades",
    image: "images/productos/black-aguja.png",
    alt: "Aguja Black Mambo 9RLT para tatuaje",
    shortDescription: "Aguja Black Mambo línea RLT para tatuaje.",
    description: [
      "Las agujas de acero quirúrgico de BLACK MAMBO están creadas con los más altos estándares de calidad.",
      "El mejor acero y la aleación perfecta, creado con la más alta precisión en Alemania.",
      "El apriete en la agrupación de las mismas dará una alta solidez en cualquier tipo de trabajo.",
      "El desgaste y el despunte no serán un problema incluso en grandes trabajos.",
      "Nuestro estricto control de calidad al inicio y fin de serie mediante un palpado de precisión a la micra garantiza a nuestras puntas un ajuste y calibrado perfecto.",
      "Esto hace que las agujas BLACK MAMBO PREMIUM cumplan con los más altos estándares de calidad.",
    ],
    tags: ["agujas", "tatuaje", "black mambo", "rlt"],
    variants: [
      { label: "1009RLT", price: 27, sku: "SAN-BLACK-MAMBO-RLT-9RLT-1009RLT" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "black-mambo-rs-7rs",
    sku: "SAN-BLACK-MAMBO-RS-7RS",
    name: "Aguja Black Mambo 7RS",
    slug: "black-mambo-rs-7rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "black-mambo",
    needleLine: "rs",
    price: 26,
    currency: "EUR",
    unit: "20 unidades",
    image: "images/productos/black-aguja.png",
    alt: "Aguja Black Mambo 7RS para tatuaje",
    shortDescription: "Aguja Black Mambo línea RS para tatuaje.",
    description: [
      "Las agujas de acero quirúrgico de BLACK MAMBO están creadas con los más altos estándares de calidad.",
      "El mejor acero y la aleación perfecta, creado con la más alta precisión en Alemania.",
      "El apriete en la agrupación de las mismas dará una alta solidez en cualquier tipo de trabajo.",
      "El desgaste y el despunte no serán un problema incluso en grandes trabajos.",
      "Nuestro estricto control de calidad al inicio y fin de serie mediante un palpado de precisión a la micra garantiza a nuestras puntas un ajuste y calibrado perfecto.",
      "Esto hace que las agujas BLACK MAMBO PREMIUM cumplan con los más altos estándares de calidad.",
    ],
    tags: ["agujas", "tatuaje", "black mambo", "rs"],
    variants: [
      { label: "1007RS", price: 26, sku: "SAN-BLACK-MAMBO-RS-7RS-1007RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "black-mambo-rs-11rs",
    sku: "SAN-BLACK-MAMBO-RS-11RS",
    name: "Aguja Black Mambo 11RS",
    slug: "black-mambo-rs-11rs",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "black-mambo",
    needleLine: "rs",
    price: 27,
    currency: "EUR",
    unit: "20 unidades",
    image: "images/productos/black-aguja.png",
    alt: "Aguja Black Mambo 11RS para tatuaje",
    shortDescription: "Aguja Black Mambo línea RS para tatuaje.",
    description: [
      "Las agujas de acero quirúrgico de BLACK MAMBO están creadas con los más altos estándares de calidad.",
      "El mejor acero y la aleación perfecta, creado con la más alta precisión en Alemania.",
      "El apriete en la agrupación de las mismas dará una alta solidez en cualquier tipo de trabajo.",
      "El desgaste y el despunte no serán un problema incluso en grandes trabajos.",
      "Nuestro estricto control de calidad al inicio y fin de serie mediante un palpado de precisión a la micra garantiza a nuestras puntas un ajuste y calibrado perfecto.",
      "Esto hace que las agujas BLACK MAMBO PREMIUM cumplan con los más altos estándares de calidad.",
    ],
    tags: ["agujas", "tatuaje", "black mambo", "rs"],
    variants: [
      { label: "1011RS", price: 27, sku: "SAN-BLACK-MAMBO-RS-11RS-1011RS" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "black-mambo-cm-9cm",
    sku: "SAN-BLACK-MAMBO-CM-9CM",
    name: "Aguja Black Mambo 9CM",
    slug: "black-mambo-cm-9cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "black-mambo",
    needleLine: "cm",
    price: 16.99,
    currency: "EUR",
    unit: "10 unidades",
    image: "images/productos/black-aguja.png",
    alt: "Aguja Black Mambo 9CM para tatuaje",
    shortDescription: "Aguja Black Mambo Magnum Curva CM para tatuaje.",
    description: [
      "Las agujas de acero quirúrgico de BLACK MAMBO están creadas con los más altos estándares de calidad.",
      "El mejor acero y la aleación perfecta, creado con la más alta precisión en Alemania.",
      "El apriete en la agrupación de las mismas dará una alta solidez en cualquier tipo de trabajo.",
      "El desgaste y el despunte no serán un problema incluso en grandes trabajos.",
      "Nuestro estricto control de calidad al inicio y fin de serie mediante un palpado de precisión a la micra garantiza a nuestras puntas un ajuste y calibrado perfecto.",
      "Esto hace que las agujas BLACK MAMBO PREMIUM cumplan con los más altos estándares de calidad.",
    ],
    tags: ["agujas", "tatuaje", "black mambo", "cm"],
    variants: [
      { label: "1009CM", price: 16.99, sku: "SAN-BLACK-MAMBO-CM-9CM-1009CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "black-mambo-cm-15cm",
    sku: "SAN-BLACK-MAMBO-CM-15CM",
    name: "Aguja Black Mambo 15CM",
    slug: "black-mambo-cm-15cm",
    category: "agujas",
    categoryLabel: "Agujas",
    brand: "black-mambo",
    needleLine: "cm",
    price: 15.99,
    currency: "EUR",
    unit: "10 unidades",
    image: "images/productos/black-aguja.png",
    alt: "Aguja Black Mambo 15CM para tatuaje",
    shortDescription: "Aguja Black Mambo Magnum Curva CM para tatuaje.",
    description: [
      "Las agujas de acero quirúrgico de BLACK MAMBO están creadas con los más altos estándares de calidad.",
      "El mejor acero y la aleación perfecta, creado con la más alta precisión en Alemania.",
      "El apriete en la agrupación de las mismas dará una alta solidez en cualquier tipo de trabajo.",
      "El desgaste y el despunte no serán un problema incluso en grandes trabajos.",
      "Nuestro estricto control de calidad al inicio y fin de serie mediante un palpado de precisión a la micra garantiza a nuestras puntas un ajuste y calibrado perfecto.",
      "Esto hace que las agujas BLACK MAMBO PREMIUM cumplan con los más altos estándares de calidad.",
    ],
    tags: ["agujas", "tatuaje", "black mambo", "cm"],
    variants: [
      { label: "1015CM", price: 15.99, sku: "SAN-BLACK-MAMBO-CM-15CM-1015CM" },
    ],
    featured: false,
    offer: false,
    active: true,
  },
  {
    id: "camiseta-sanitattoo-blanca",
    sku: "SAN-MERCH-CAMISETA-BLANCA",
    name: "Camiseta blanca SANITATTOOSUPPLY",
    slug: "camiseta-sanitattoo-blanca",
    category: "merchandising",
    categoryLabel: "merchandising",
    brand: "sanitattoo",
    price: 20,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/merchandising/cami-blanca.png",
    alt: "Camiseta de merchandising SANITATTOO",
    shortDescription: "Camiseta oficial SanitattooSupply. Consulta tallas y stock por WhatsApp.",
    description:[
      "Camiseta oficial SanitattooSupply confeccionada en algodón premium para máxima comodidad y durabilidad.",
      "Diseño minimalista con identidad propia, perfecta para dentro y fuera del estudio.",
      "- 100% Algodón.",
      "- Corte oversize",
      "- Disponible en varias tallas.",
      "- Edición exclusiva."
    ],
    tags: ["merchandising", "camiseta", "textil"],
    featured: false,
    offer: false,
    active: true,
    priceIncludesVat: true,
    merchandisingFeatured: true,
    gallery: [
      "images/merchandising/cami-blanca.png",
      "images/merchandising/cami-blanca-atras.png",
    ],
    variants: [
      { label: "S", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-S" },
      { label: "M", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-M" },
      { label: "L", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-L" },
      { label: "XL", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-XL" },
    ],
  },
  {
    id: "camiseta-sanitattoo-negra",
    sku: "SAN-MERCH-CAMISETA-NEGRA",
    name: "Camiseta negra SANITATTOOSUPPLY",
    slug: "camiseta-sanitattoo-negra",
    category: "merchandising",
    categoryLabel: "merchandising",
    brand: "sanitattoo",
    price: 20,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/merchandising/cami-negra-.png",
    alt: "Camiseta negra de merchandising SANITATTOO",
    shortDescription: "Camiseta oficial SanitattooSupply. Consulta tallas y stock por WhatsApp.",
    description:[
      "Camiseta oficial SanitattooSupply confeccionada en algodón premium para máxima comodidad y durabilidad.",
      "Diseño minimalista con identidad propia, perfecta para dentro y fuera del estudio.",
      "- 100% Algodón.",
      "- Corte oversize",
      "- Disponible en varias tallas.",
      "- Edición exclusiva."
    ],
    tags: ["merchandising", "camiseta", "textil"],
    featured: false,
    offer: false,
    active: true,
    priceIncludesVat: true,
    merchandisingFeatured: true,
    gallery: [
      "images/merchandising/cami-negra-.png",
      "images/merchandising/cami-negra-atras.png",
    ],
    variants: [
      { label: "S", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-S" },
      { label: "M", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-M" },
      { label: "L", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-L" },
      { label: "XL", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-XL" },
    ],
  },
  {
    id: "camiseta-sanitattoo-nueva-blanca",
    sku: "SAN-MERCH-CAMISETA-NUEVA-BLANCA",
    name: "Camiseta SANITATTOOSUPPLY 2026",
    slug: "camiseta-sanitattoo-nueva-blanca",
    category: "merchandising",
    categoryLabel: "merchandising",
    brand: "sanitattoo",
    price: 20,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/merchandising/cami-nueva.png",
    alt: "Camiseta negra de merchandising SANITATTOO",
    shortDescription: "Camiseta oficial SanitattooSupply nueva edición 2026.",
    description:[
      "Nueva edición SanitattooSupply con diseño renovado y esencia exclusiva.",
      "Una pieza creada para representar la marca con estilo y personalidad.",
      "- 100% Algodón.",
      "- Corte oversize",
      "- Disponible en varias tallas.",
      "- Edición exclusiva."
    ],
    tags: ["merchandising", "camiseta", "textil"],
    featured: false,
    offer: false,
    active: true,
    priceIncludesVat: true,
    merchandisingFeatured: false,
    badge: "Próximamente",
    gallery: [
      "images/merchandising/cami-nueva.png",
      "images/merchandising/cami-nueva-atras.png",
    ],
    variants: [
      { label: "S", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-S" },
      { label: "M", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-M" },
      { label: "L", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-L" },
      { label: "XL", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-XL" },
    ],
  },
  {
    id: "camiseta-sanitattoo-nueva-negra",
    sku: "SAN-MERCH-CAMISETA-NUEVA-NEGRA",
    name: "Camiseta SANITATTOOSUPPLY 2026",
    slug: "camiseta-sanitattoo-nueva-negra",
    category: "merchandising",
    categoryLabel: "merchandising",
    brand: "sanitattoo",
    price: 20,
    currency: "EUR",
    unit: "1 unidad",
    image: "images/merchandising/cami-nueva-negra.png",
    alt: "Camiseta negra de merchandising SANITATTOO",
    shortDescription: "Camiseta oficial SanitattooSupply nueva edición 2026.",
    description:[
      "Nueva edición SanitattooSupply con diseño renovado y esencia exclusiva.",
      "Una pieza creada para representar la marca con estilo y personalidad.",
      "- 100% Algodón.",
      "- Corte oversize",
      "- Disponible en varias tallas.",
      "- Edición exclusiva."
    ],
    tags: ["merchandising", "camiseta", "textil"],
    featured: false,
    offer: false,
    active: true,
    priceIncludesVat: true,
    merchandisingFeatured: true,
    badge: "Próximamente",
    gallery: [
      "images/merchandising/cami-nueva-negra.png",
      "images/merchandising/cami-nueva-negra-atras.png",
    ],
    variants: [
      { label: "S", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-S" },
      { label: "M", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-M" },
      { label: "L", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-L" },
      { label: "XL", price: 20, sku: "SAN-MERCH-CAMISETA-BLANCA-XL" },
    ],
  },
];

export function getActiveProducts() {
  return products.filter((p) => p.active);
}

export function getCatalogProducts() {
  return getActiveProducts().filter((p) => p.brand !== "sanitattoo");
}

export function getMerchandisingProducts() {
  return getActiveProducts().filter((p) => p.brand === "sanitattoo");
}
